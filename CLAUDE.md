...
プレビュー環境 https://preview.smkc.bluemoon.works/ に対して定期的にE2Eテストを実施し、問題を発見・修正・デプロイ・検証する。
E2Eは本番D1を共有しない preview 専用D1を対象にする。本番URLを指定したE2Eは `E2E_ALLOW_PRODUCTION=1` がない限り起動時に失敗させる。
...
const browser = await chromium.launchPersistentContext(
  '/tmp/playwright-smkc-preview-profile',
...
- `pkill -f "chromium"` は禁止。ブラウザプロセスの誤殺リスクがある。Playwrightプロファイルの問題は `rm -f /tmp/playwright-smkc-preview-profile/SingletonLock` で対応すること。
