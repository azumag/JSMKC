...
## 完了したタスク (2026-01-24)
✅ [Issue #124: Fix logger mock architecture causing 100+ test failures](https://github.com/azumag/JSMKC/issues/124)

### 実装完了

すべての43個のAPIルートについて、logger作成をモジュールレベルから関数レベルに移動

### 修正したファイル

**トーナメントモジュール**:
- MR (Match Race): 10ファイル
- GP (Grand Prix): 7ファイル  
- BM (Battle Mode): 10ファイル
- TA (Time Attack): 3ファイル
- TT (Tournament Token): 1ファイル
...
**根本原因**: APIルートでloggerインスタンスがモジュールレベルで作成されており、Jestのテストモックが設定される前に作成されていた

**解決策**: logger作成を各ハンドラー関数内に移動

### テスト状況

Loggerモック問題は解決されました。`loggerMock.error`および`loggerMock.warn`の呼び出しを期待していたテストが通過するようになります。

**注**: ロガー関係のテストは修正されましたが、他のテスト期待値の不一致が残っている場合があります。それらは別途対処が必要です。

詳細は `LOGGER_FIX_IMPLEMENTATION_SUMMARY.md` を参照してください。
...
**重要**: Issue #121は完了しましたが、新たにIssue #124を作成しました。この問題は100以上のテストをブロックしており、最優先で対処が必要です。

### 問題の概要

複数のAPIテストがloggerモックのアーキテクチャ問題で失敗中。テストが`logger.error`や`logger.warn`が呼ばれることを期待しているが、これらの呼び出しが記録されていない。

### 根本原因

APIルートでloggerインスタンスがモジュールレベルで作成されている：
...
3. テストが `loggerMock.error` が呼ばれることを期待 → 間違ったインスタンスをチェック

### 影響範囲

- **推定100+テスト**がこの問題でブロックされている
- 全トーナメントモジュールに影響（GP、BM、MR、TA、TT）
- 現在のテストパス率: ~53% (171/318テストがパス)

### 推奨される解決策

**解決策 A: 関数レベルのLogger作成（推奨）**
すべての30+ APIルートでlogger作成を関数レベルに移動

```typescript
export async function GET(request, { params }) {
  const logger = createLogger('mr-api'); // 関数内に移動
...
1. すべてのloggerモックテストがパス（約100+テストが修正される）
2. テストパス率が~53%から80%+に改善
3. APIルートのパフォーマンス低下なし
4. 既存のパス中のテストが引き続きパス
...
テスト期待値の不一致を調査し、部分的に修正完了。しかし、loggerモックのアーキテクチャ問題（Issue #124）により、多くのテストが依然として失敗中。
