...
The manual `Security audit review` workflow collects six local Prisma 7 migration probes after the compatible and next-major upstream checks:
...
The Job summary must still make advisory probe failures visible. The main check table therefore records the GitHub Actions outcome for all six Prisma 7 probes in addition to the lockfile, exception-status, canonical-audit, compatible-upstream, and next-major checks. This distinguishes two different situations that would otherwise look similar in a saved summary: a probe successfully reporting migration blockers, and a probe failing before it could produce trustworthy evidence.
