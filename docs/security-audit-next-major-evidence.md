...
package-set reason は state だけでは区別できない停止理由を機械可読に残します。`upstream-remediation-not-applicable` は next-major 自体がまだ vulnerable、`prisma-client-registry-unavailable` は candidate と同じ `@prisma/client` を確認できない状態、`adapter-d1-registry-unavailable` は candidate と一致する client は確認できたが D1 adapter の確認に失敗した状態、`runtime-package-version-mismatch` は registry evidence が candidate version と一致しない状態、`published-package-set-ready` は CLI/client/D1 adapter が同じ stable version で揃った状態です。lookup 失敗や不整合は候補を出さず fail-closed な advisory evidence として扱います。
