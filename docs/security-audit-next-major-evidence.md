...
GitHub Actions output には `published_remediation_candidate=<version|none>` に加えて、`published_remediation_package_set_state=<ready|incomplete|unavailable|not-applicable>`、`published_remediation_prisma_client_version`、`published_remediation_adapter_d1_version` を出します。Job summary でも `Published remediation candidate` と並べて package-set state、候補 `@prisma/client` version、候補 `@prisma/adapter-d1` version を表示します。候補version自体は package set が `ready` の場合だけ出るため、reviewer は upstream branch merge、CLIだけの公開、client/adapterの部分公開を installable migration set と取り違えずに判断できます。これらの package-set output は summary 専用の advisory evidence であり、`compatible_upstream_gate` には渡しません。
