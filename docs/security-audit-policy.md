...
`scripts/security-audit.js` は `npm audit --json` を読み取り、high / critical finding が無い場合だけ通常成功する。期限付きの既知例外を helper 内に持つ場合も、例外条件は依存バージョン・lockfile 上の属性・dev-only 条件・advisory ID と canonical URL・affected range・severity・dependency graph まで狭く固定する。
...
- 許可対象の advisory ID / canonical URL / affected range、severity、依存バージョン、dependency graph が変化する
...
- `smkc-score-app/__tests__/scripts/security-audit.test.ts`: fail-closed helper の許可条件と、advisory の canonical URL・affected range・severity・`via` / `effects` topology、root devDependency 宣言を含む前提が変化した場合の blocking 動作を検証する。
