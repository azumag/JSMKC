...
また、一時例外が成立する利用文脈は manifest と lockfile の両方で固定する。helper は実際の `package.json` を1回だけ読み、その同じ manifest snapshot を packageManager の runtime pin 検証と Prisma の一時例外判定の両方で共有する。これにより guard 間で `package.json` を読み直して異なる内容を評価する余地をなくす。さらに `dependencies` / `devDependencies` / `optionalDependencies` / `peerDependencies` の宣言 map は、`package.json` と `package-lock.json` の root package snapshot で完全一致することを audit subprocess 起動前に要求する。片側だけの追加・削除、range drift、非 object container、非 string range は stale / malformed lockfile として fail-closed にし、helper を単独実行した場合でも古い dependency snapshot に対する監査を成功扱いしない。そのうえで Prisma が現在確認済みの devDependency range で宣言され、production dependency には存在しないことを要求し、実際にインストールされた Prisma とその設定パッケージの version・`resolved`・`integrity`・`devOptional` 属性、およびそこから許可対象の推移的依存へ至る dependency edge を現在確認済みの組み合わせに一致させる。manifest の許容範囲内で Prisma が更新された場合や、同じ version 名でも Prisma 側 artifact の供給元または内容が変化した場合は、実装や設定読み込み経路が変化していないかを再評価するまで既知例外を自動継続しない。

この例外判定は npm package-lock v3 の `packages` map とその属性意味論に依存するため、CI は audit helper の前に `scripts/security-audit-lockfile.js` を実行し、`security-audit.js` も同じ shared validator を audit subprocess 起動前に再実行する。`package-lock.json` の top-level が object であり、`lockfileVersion` が現在の `3`、`packages` が non-array object、かつ `packages[""]` の root package snapshot が non-array object で、root dependency snapshot が実 manifest と一致する場合だけ監査へ進む。これにより helper を単独実行するローカル・別CI経路でも lockfile guard や manifest/lockfile parity guard を迂回できない。lockfile schema が更新・欠落・破損した場合や root dependency snapshot が stale な場合は、同じフィールド名が残っていても意味論や監査対象が変化している可能性があるため、例外を再評価するまで fail-closed にする。
...
- `package-lock.json` が v3 object schema でなくなる、`lockfileVersion` が `3` 以外になる、`packages` map が欠落・非 object / array になる、`packages[""]` の root package snapshot が欠落・非 object / array になる、または root の `dependencies` / `devDependencies` / `optionalDependencies` / `peerDependencies` が実 `package.json` と一致しない
...
- 実 `package.json` と `package-lock.json` root snapshot の dependency declaration が一致しない、dependency map が object 以外へ drift する、または range が非 string になる。特に Prisma の production/dev-only 境界、インストール済み Prisma chain の version・dev-only 属性、lockfile 上の依存エッジが変化した場合は再評価を要求する
...
- `smkc-score-app/__tests__/scripts/security-audit-runtime-guard.test.ts`: `security-audit.js` が `package.json` の manifest snapshot を取得し、その同じ snapshot で pinned npm runtime verifier と manifest/lockfile parity guard を通過した後に `npm audit` subprocess へ進むことを検証し、直接実行時の guard bypass、guard 間の manifest 再読込、stale lockfile に対する audit 起動を防ぐ。
- `smkc-score-app/__tests__/scripts/security-audit-lockfile.test.ts`: 実リポジトリの package-lock が v3 object schema と object 型の root package snapshot を満たし、4種の root dependency map が実 manifest と一致すること、schema version・top-level / `packages` container・root snapshot・dependency declaration の drift を拒否すること、CI が schema preflight を audit helper より先に実行することを検証する。
