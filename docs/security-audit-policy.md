...
各 vulnerability entry の optional `fixAvailable` も監査結果の意味論として検証する。boolean または `name` / `version` の non-empty string と boolean `isSemVerMajor` を持つ object だけを受理し、その他の型・欠落 metadata は schema drift として `invalid-audit-report` にする。一時例外の blocking chain では、`fixAvailable: true` または `isSemVerMajor: false` の remediation が現れた時点で既知例外を停止する。つまり通常の `npm audit fix` で適用可能な非破壊 remediation が利用可能になった場合は、脆弱性を放置したまま allowlist を継続せず、依存更新を取り込むまで CI を fail-closed にする。`fixAvailable: false`、field 省略、または semver-major remediation しか無い場合だけ現在の例外条件を継続できる。
...
- 許可対象 blocking chain に non-breaking な `fixAvailable` remediation が現れる、または `fixAvailable` metadata の型・必須 field が既知 schema から drift する
...
- `smkc-score-app/__tests__/scripts/security-audit-fix-availability.test.ts`: optional `fixAvailable` の schema を検証し、semver-major remediation または remediation なしでは一時例外を維持する一方、通常の `npm audit fix` で適用できる non-breaking remediation が現れた場合は fail-closed にする契約を固定する。
