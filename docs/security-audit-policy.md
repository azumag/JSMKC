...
- `smkc-score-app/__tests__/scripts/security-audit-runtime-guard.test.ts`: `security-audit.js` が `package.json` の manifest snapshot を取得し、その同じ snapshot で pinned npm runtime verifier を呼んだ後に `npm audit` subprocess へ進むことを検証し、直接実行時の guard bypass と guard 間の manifest 再読込を防ぐ。
