...
compatible upstream probe は canonical npm registry に対する read-only `npm view` だけを使用します。現在の `devDependencies.prisma` selector に合致する version 群から prerelease を除外した最新 stable version を選び、その exact version の `@prisma/config` metadata を確認します。stable version が1件も得られない場合は候補を推測せず非0終了します。`deepmerge-ts` requirement が patched line（8.0.0以上）なら `compatible-forward-remediation-available`、まだ vulnerable line なら `compatible-release-still-vulnerable` と報告します。前者は「現在の manifest selector の範囲内に再評価候補となる stable release が現れた」ことだけを意味し、依存更新や一時例外削除を自動実行しません。
