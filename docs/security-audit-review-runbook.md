...
4. `security-audit-upstream.js` — `package.json` の現在の Prisma version selector をそのまま使って canonical npm registry を照会し、その selector の範囲内で入手できる最新 Prisma と対応する `@prisma/config → deepmerge-ts` requirement を確認する。依存を変更せず、同一 major / version range 内に安全な forward release が配布されたかだけを観測する。
...
compatible upstream probe は canonical npm registry に対する read-only `npm view` だけを使用します。現在の `devDependencies.prisma` selector に合致する version 群から最新 version を選び、その exact version の `@prisma/config` metadata を確認します。`deepmerge-ts` requirement が patched line（8.0.0以上）なら `compatible-forward-remediation-available`、まだ vulnerable line なら `compatible-release-still-vulnerable` と報告します。前者は「現在の manifest selector の範囲内に再評価候補が現れた」ことだけを意味し、依存更新や一時例外削除を自動実行しません。
...
`compatible-forward-remediation-available` が出た場合も、自動更新は行いません。manifest selector 内で新しい candidate が観測されたという判断材料として扱い、candidate version へ明示的に依存更新したPRで lockfile、status、canonical audit、unit tests、lint、format、Prisma/D1 parity、Cloudflare build を確認します。`compatible-release-still-vulnerable` の場合は、現在の selector の範囲内には #3114 を解消する upstream package がまだ配布されていません。
