...
これにより、改行による複数行化、ANSI escape による端末表示の汚染、workflow-command 風の文字列が独立したログ行として見えること、巨大な npm error body や parser input snippet による監査ログの肥大化を防ぎつつ、原因調査に必要な診断内容は可視文字列として保持します。
