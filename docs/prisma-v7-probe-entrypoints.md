...
aggregate は child probe を開始する前に probe manifest 自体も検証し、空の manifest、malformed entry、重複 key、重複 script を fail-closed で拒否します。特に重複 key は `Object.fromEntries` による evidence の暗黙上書きを起こし得るため、manifest 不整合時には child probe を一つも実行せず終了します。

machine-readable aggregate では child probe ごとに 30 秒の実行上限と 1 MiB の stdout/stderr buffer 上限を明示しています。将来 probe が外部I/Oや子プロセスを追加して停止した場合でも review automation を無期限に占有せず、異常に大きな出力を生成した場合も親プロセスが無制限に保持しません。timeout、buffer overflow、signal termination はいずれも operational failure として partial evidence なしで fail-closed にします。

いずれかの probe が実行不能、timeout、buffer overflow、signal termination、非0終了、空出力、invalid JSON、JSON object 以外を返した場合、aggregate command は partial evidence を出力せず非0終了します。これにより自動処理が欠落した evidence を完全な review 結果として扱うことを防ぎます。aggregate JSON 自体にも時刻等の非決定的フィールドは追加しないため、同じ repository state の比較にも利用できます。
