...
nightly E2E も同じ `packageManager` pin を `npm ci` より前に適用する。E2E は security audit 自体を実行しないが、runner 更新だけで lockfile 解釈・lifecycle script・`postinstall` の npm runtime が通常 CI とずれることを防ぐためである。通常 CI と nightly E2E の `npm ci` では local Git hook を必要としないため、`HUSKY=0` を設定して prepare-time の hook setup/noise を抑止する。
