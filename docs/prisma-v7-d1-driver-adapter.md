...
Import, adapter-construction, and `PrismaClient` constructor matches must also begin in executable source rather than inside a single-quoted, double-quoted, or template literal. Migration examples embedded as strings can contain syntactically convincing Prisma 7 code, but they are documentation rather than runtime wiring and therefore cannot contribute readiness evidence. The lexical guard is intentionally conservative: source that cannot be proven to contain executable adapter wiring fails closed instead of allowing quoted examples to produce `ready: true`.
