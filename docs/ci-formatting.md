...
ファイル名は shell command string へ展開せず child-process の argument 配列として渡すため、空白を含む path も同じ経路で扱う。複数ファイルは個別に診断し、あるファイルの parser error や診断生成失敗が別ファイルの修正差分表示を妨げない。診断生成自体に失敗した場合も、元の formatting failure は成功へ変換しない。
