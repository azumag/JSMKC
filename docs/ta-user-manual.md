...
マニュアル内のURLに含まれる `[id]` は、大会ごとの識別子です。自動生成IDの場合もあれば、`jsmkc2026` のようなカスタムURLが設定されている場合もあります。

**アクセス方法:**

1. 大会主催者から共有されたURLを直接開く（推奨）
2. または、大会の識別子が分かっている場合、`[id]` を実際の値に置き換えて入力

**例:**
- 大会IDが `123` の場合: `https://your-domain.com/tournaments/123/ta`
- カスタムURLが `jsmkc2026` の場合: `https://your-domain.com/tournaments/jsmkc2026/ta`

> **注意**: 参加者は主催者から共有されたURLをそのまま使ってください。カスタムURLの設定は管理者のみが行えます。
...
> **注意**: フェーズ1/2/3のいずれかが開始された後は、予選タイムの修正は管理者のみ可能です。
> **注意**: まだ未登録の場合でも、フェーズ1/2/3開始後は予選への参加登録はできません。
...
- 全20コースが入力されていない場合、合計タイムが未確定となり、全コース入力済みの選手より下位にランクされます
- **正確な順位を確認するには、全20コースの入力が必要です**

> **注意**: 部分入力の段階では、入力済みのコースでポイントは計算されますが、合計タイムが未確定のため、順位は暫定です。全コース入力完了後に正式な予選順位が確定します。
...
- **ノックアウト開始後**: フェーズ1/2/3のいずれかが開始された後は、予選タイムの修正は管理者のみ可能です

タイムの修正は、予選フリーズ前かつノックアウト開始前であれば可能です。同じコースのタイムを上書きして更新できます。ノックアウト開始後に修正が必要な場合は、管理者に依頼してください。
...
> **タイ処理**: 同タイムの選手は同着グループ最上位順位のポイントを共有します（CDM Excel の RANK 方式。次の順位はスキップされます）。
...
予選ステージでは、2人1組でペアを組んでタイム計測を行います。

#### ペアの決め方

- 管理者が各選手に**シード番号**（実力の指標、小さいほど上位）を設定します
- シード番号に基づいて、上位選手と下位選手がバランスよくペアになるように自動計算されます
- 例：8名の場合、シード1位と8位、2位と7位、3位と6位、4位と5位がペアになります

#### ペア走行のルール

- ペア相手のタイムも代理入力が可能です（自分のタイムだけでなくパートナーのタイムも入力できます）
- ペアの変更は管理者のみ可能です

> **注意**: ペアの組み合わせはランキングページの「ペア管理」には選手からはアクセスできません。割り当ては管理者が行います。
...
| シード | 選手の実力指標（番号が小さいほど上位）。ペア走行の組み合わせに使用 |
| ペア走行 | 2人1組でタイム計測を行う形式（§3.1）。パートナーのタイムも代理入力可能 |
...
| 1.2 | 2026-02-08 | 部分入力時の合計タイム未確定動作を正確に記載 |
| 1.3 | 2026-02-08 | タブ間キャッシュ共有の不正確な記述を修正 |
| 1.4 | 2026-04-09 | ペア走行（§3.1）のルール・シード番号の説明を追加、用語集にペア走行を追加 |
