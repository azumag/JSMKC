...
参加者がいないフェーズは自動的にスキップされます：

- 例：フェーズ1の対象者が不在の場合、フェーズ2に直接進む可能性があります
...
1. 「エクスポート」ボタンをクリック
2. CSVファイルが自動的にダウンロードされます（UTF-8 BOM付き、Excel対応）

### 6.2 エクスポート内容

CSVファイルには以下の7列が含まれます：
...
| 1.2 | 2026-02-08 | エクスポート形式をCSVに修正、フリーズ確認ダイアログ記述修正、CSVカラム詳細追記 |
