...
1. **合計ポイント（qualificationPoints）**: 全20コースのポイント合計（四捨五入）が高い順
...
- 同タイムの選手: 同着グループ最上位順位のポイントを共有（CDM Excel の RANK 方式。次の順位はスキップされる）
- 全20コースの合計は四捨五入（CDM Excel ワークブックの整数表示と一致、issue #2768）
...
順位や敗退を確定できない場合、対象選手だけでサドンデスを実施します。目的によってコースの扱いが異なります（issue #2773）。

| シナリオ | 発生条件 | サドンデスのコース | コースプールへの影響 |
|----------|----------|--------------------|----------------------|
| ライフ喪失同着 | ライフ減少対象と安全圏の境界に同タイムがまたがる（一部だけがライフを失う） | **ベースラウンドと同じコースを2回目走行** | 影響なし（消費しない） |
| リバイバル | リセット閾値越えでゼロライフ候補が上限を超える（トップ8/トップ4進出者を決定） | 新規コース | プールから消費 |
| 銅メダル決定 | トップ4段階で下位2名が同時に最後のライフを失う（3位決定戦） | 新規コース | プールから消費 |

- ライフ喪失同着のサドンデスは、境界にまたがった同タイム勢が**同じコースをもう一度走り**、2回目のタイムでライフを失う選手を決めます。今後のラウンドのコース候補には影響しません。
- 銅メダル決定は同着でなくても発生します（下位2名がともに残りライフ1なら、通常のタイム順でも両者が同時敗退となるため）。両者は新規コースで走り、速い方が3位（銅メダル）、残り2名はトップ2へ進出（ライフはリセット）します。
- サドンデスで再び決着がつかない場合は、同じ規則でサドンデスが継続されます（ライフ喪失同着なら同一コースを再々走行）。
- コースは管理画面で変更できますが、ライフ喪失同着のサドンデスではベースコースを選び直すこともできます（他の既走コースは選べません）。
...
### 8.5 予選ポイントの再計算（管理者メンテナンス）

得点計算ルールの変更をデプロイした後、既存の予選結果に新しいルールを反映する場合は、管理者として次のAPIを1回実行します。

```http
POST /api/tournaments/{tournamentId}/ta
Content-Type: application/json

{"action":"recalculate_qualification"}
```

この操作は、対象大会の予選エントリーすべてについて、コース別ポイント（`courseScores`）、予選ポイント（`qualificationPoints`）、合計値、順位を保存済みタイムから再計算します。管理者権限が必要で、実行内容は監査ログに記録されます。

> **注意**: 大会IDと対象環境を確認し、同時にタイム編集が行われていない状態で実行してください。
...
| 1.5 | 2026-07-10 | 得点計算ルール変更後に既存の予選ポイントを再計算する管理者APIを追記 |
