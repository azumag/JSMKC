...
GitHub REST API request には30秒の timeout を設定し、外部API応答待ちで review job が無期限に停止しないようにします。timeout・network error・HTTP error はいずれも evidence 未取得として fail-closed で扱います。
