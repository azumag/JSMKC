...
probe は GitHub REST API から issue #30052 を取得し、issue number、`open` / `closed` state、最終更新日時、canonical issue URL だけを採用します。API response が失敗する、JSON として読めない、issue number・state・timestamp・URL が期待する tracking issue と一致しない場合は fail-closed で終了します。
