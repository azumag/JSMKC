...
The probe writes its detailed readiness table to the GitHub Actions job summary, while the main #3114 summary records the step outcome. This keeps a manual review self-contained: reviewers can see both whether a patched Prisma 7 dependency chain exists upstream and which local migration prerequisites still need work before an explicit migration PR is safe to attempt.
