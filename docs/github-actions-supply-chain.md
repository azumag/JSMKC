...
The same least-privilege rule applies to the workflow token itself. Regular CI and the PR review compatibility workflow declare `contents: read` as the workflow-level default instead of inheriting the repository's mutable `GITHUB_TOKEN` default. The PR compatibility workflow grants `contents: write` and `pull-requests: write` only to its dedicated auto-merge compatibility job; lint/test and Cloudflare compatibility validation inherit the read-only default. `smkc-score-app/__tests__/docs/validation-workflow-permissions.test.ts` keeps this boundary under regression coverage.
