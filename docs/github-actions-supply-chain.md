...
The same least-privilege rule applies to the workflow token itself. Regular CI and the PR review compatibility workflow declare `contents: read` as the workflow-level default instead of inheriting the repository's mutable `GITHUB_TOKEN` default. While #3499 is unresolved, the compatibility workflow must remain read-only and must not enable auto-merge: `main` currently has no enforced required status checks, so workflow-local validation alone is not a repository-level merge guarantee. `smkc-score-app/__tests__/docs/validation-workflow-permissions.test.ts` keeps both the read-only permission boundary and the absence of `gh pr merge --auto` under regression coverage. If auto-merge is restored later, first enable and verify required checks at the repository policy layer.

For #3499, standard CI deliberately owns the unqualified check names `Lint & Test` and `Prisma / D1 migration parity`. The PR compatibility workflow uses `PR Compatibility Lint & Test` instead of duplicating `Lint & Test`, so the repository policy can select the standard CI checks without ambiguity. `smkc-score-app/__tests__/docs/required-status-check-names.test.ts` keeps that namespace separation under regression coverage. When branch protection is enabled, prefer the two standard CI check names above as the initial required set; keep the compatibility workflow advisory unless repository policy explicitly decides otherwise.

After changing the repository policy, verify the live `main` branch state with `node smkc-score-app/scripts/verify-required-status-checks.cjs`. The verifier reads the public branch endpoint, accepts either legacy `contexts` entries or app-bound `checks` entries, and exits non-zero unless branch protection is enabled, required-check enforcement is active, and both standard CI checks are present. `GH_TOKEN` or `GITHUB_TOKEN` is optional for the public repository but can be supplied to avoid anonymous API rate limits. Use `--json` for machine-readable evidence. Until #3499 is configured, a non-zero exit is the expected result rather than a reason to weaken the verifier.

Standard CI also treats `.github/workflows/**` as a validation path, in addition to application files under `smkc-score-app/**`. This ensures a workflow-only pull request cannot bypass the full lint, formatting, unit-test, security-audit, and migration-parity validation suite merely because it does not touch application code. `smkc-score-app/__tests__/docs/ci-workflow-trigger-paths.test.ts` keeps both the push and pull-request path filters under regression coverage.
