...
- author association は GitHub が定義する `COLLABORATOR` / `CONTRIBUTOR` / `FIRST_TIMER` / `FIRST_TIME_CONTRIBUTOR` / `MANNEQUIN` / `MEMBER` / `NONE` / `OWNER` のいずれかであること。単に大文字と underscore だけで構成された未知値は受理しない
