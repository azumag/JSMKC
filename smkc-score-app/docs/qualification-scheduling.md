...
レスポンスには `modes` と同じ診断データから計算した `summary` も含まれます。管理UIと同じ `Legacy circle` / CDM fixture readiness / BREAK / generation blocked の件数をAPI利用側でも再計算せず参照できるため、#3054 の仕様判断対象を運用ツールから機械的に集計できます。`legacyCirclePlayerCount` は現行policyでlegacy circleとなるグループに所属する選手数の合計で、グループ件数だけでは見えにくい実際の影響人数を示します。`legacyCircleSizeBreakdown` は legacy circle を実人数ごとにまとめ、`playerCount` / `groupCount` / `cdmFixtureCapacity` / `cdmBreakSlotCount` を返します。同じ人数のグループがBM / MR / GPをまたいで存在する場合も1つのbucketへ集約されるため、どの人数帯をCDMへ移行すると何グループへ影響し、BREAKや追加fixtureが必要になるかを直接把握できます。`legacyCircleModeBreakdown` は同じlegacy circleをBM / MR / GPごとにまとめ、各モードの対象グループ数と対象選手数に加えて、現行fixtureで切り替え可能な件数、exact-fit、BREAK必要、fixture未対応の件数を返します。これにより、人数帯だけでなく競技モード単位でも仕様変更の影響範囲を確認できます。
...
画面上部には `summarizeQualificationScheduleDiagnostics` による読み取り専用の集計も表示します。`Legacy circle` はCDM-firstだが現行policyでcircleを使うグループ数、`Legacy circle players` はその全グループに所属する選手数です。その内訳として、`Legacy circle with CDM fixture` は現行policyではcircleのままだが低レベルのCDM fixture自体は利用できるグループ数、`Legacy circle exact-fit CDM` はそのうち実人数とfixture容量が一致してBREAKを必要としないグループ数、`Legacy circle CDM with BREAK` は既存fixtureへ切り替えられるもののBREAK slotを必要とするグループ数、`Legacy circle without CDM fixture` は13名など現状のfixture対応表ではそのままCDMへ切り替えられないグループ数を示します。さらに `Legacy circle sizes` はlegacy circleだけを人数別に集約し、各人数についてグループ数、候補CDM fixture容量、必要BREAK数を1行で表示します。`Legacy circle modes` はBM / MR / GPごとに対象グループ数・対象選手数と exact-fit / BREAK必要 / fixture未対応の件数を表示し、どの競技モードに仕様変更の影響が集中するかを確認できます。これにより、#3054 の「13名以下もCDMへ切り替えるか」「BREAK配置をTTでも採用するか」という2つの判断を、追加fixtureなし・BREAKなしで移行可能な範囲、BREAK方針の決定が必要な範囲、追加fixtureまたは別方針が必要な範囲に分け、人数帯と競技モードの両面から把握できます。
...
`__tests__/lib/qualification-schedule-policy.test.ts` で現在の境界と判定理由を独立して固定しています。`__tests__/lib/qualification-schedule-diagnostics.test.ts` では複数モード・複数グループをまとめた診断結果、明示的なcircle設定の維持、#3054 の判断対象グループと対象選手数を集計するsummary、legacy circleのうち現在のCDM fixtureだけで切り替え可能なグループをBREAK不要・BREAK必要・fixture未対応に分けた内訳、BM / MR / GPをまたぐ同一人数を人数別bucketへ集約すること、および競技モード別bucketへ対象選手数・exact-fit / BREAK必要 / fixture未対応の件数を集約することを検証します。`__tests__/components/tournament/qualification-schedule-diagnostics-panel.test.tsx` では管理UIが保存された方式と実効方式の差、circle/CDMの実効方式、判定理由、判断用summaryとlegacy circleのCDM/BREAK readiness内訳、対象選手数、人数別・モード別breakdown、未対応CDM requestの警告、空モードを表示することを確認します。Issue #3054 の要件確定後は、まずpolicyテストの期待値を仕様に合わせて更新し、その後にpolicy・E2Eを変更します。
