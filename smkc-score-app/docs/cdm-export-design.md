...
loadCDMTemplate (ASSETS fetch, 現行どおり)
  ...
  → 書込対象シートの数式セルからテンプレート由来の計算済みキャッシュを除去
  → xl/workbook.xml の <calcPr> に calcMode="auto" / fullCalcOnLoad="1" / forceFullCalc="1" を付与
...
- 書込対象シートでは、数式セルそのものは保持しつつ計算済み `<v>`/`<is>` キャッシュを除去する。
  CDM 2025 完成テンプレートは古い選手名・スコアのキャッシュを持つため、Excel の再計算前に
  テンプレート残骸が表示されることを防ぐ。
...
xlsx-zip-patcher.ts   zip入出力・パート素通し・数式キャッシュ除去・calcChain除去・calcPr付与・シート解決
