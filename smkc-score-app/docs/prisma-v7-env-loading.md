...
Direct CommonJS evidence must use a bare `require` identifier. Member methods such as `loader.require('dotenv').config()`, including forms with whitespace/comments before `.require`, do not prove that Node's CommonJS loader executed and are ignored. The probe keeps scanning after non-qualifying member calls, so a later genuine `require('dotenv').config()` can still satisfy readiness.

Prisma config namespace forms are evaluation boundaries too. Both `import * as prismaConfig from 'prisma/config'; prismaConfig.defineConfig(...)` and `const prismaConfig = require('prisma/config'); prismaConfig.defineConfig(...)` are tracked. Named ESM imports and CommonJS destructuring are collected across all aliases rather than stopping at the first binding, and the earliest top-level `defineConfig` evaluation across named and namespace forms wins. This prevents a later `dotenv.config()` from being accepted merely because a different alias or namespace performs the first config evaluation.
