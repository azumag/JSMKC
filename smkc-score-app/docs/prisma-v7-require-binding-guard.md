...
A bare identifier is only trustworthy when it still refers to the module loader. A local top-level binding or reassignment can make the same source shape fail-open:
...
`prisma.config.ts` must not create or replace a top-level binding named `require`.
...
- `const`, `let`, or `var` bindings named `require`;
- object/array destructuring that binds a local name `require`;
- ESM import bindings named `require`;
- top-level function or class declarations named `require`;
- direct top-level assignment or compound assignment to bare `require`.

Direct CommonJS calls such as `require('dotenv')` remain allowed. Member methods such as `loader.require(...)`, quoted examples, comments, and bindings inside nested helper bodies are not treated as top-level loader rebindings.
