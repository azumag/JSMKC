...
A successful issue/reissue POST and local QR image rendering are separate outcomes. Once the server returns the one-time raw token, that mutation remains successful even if `QRCode.toString()` later fails in the browser. The client keeps the raw login URL visible and copyable, logs the QR rendering failure separately, and simply omits QR image/Print controls for that session. It must not misreport a local rendering failure as `failedToIssueQrCode`, because reissuing would invalidate an already valid one-time credential unnecessarily.
