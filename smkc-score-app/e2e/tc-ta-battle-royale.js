...
async function submitPhase3ResultsWithPreview(page, tournamentId, results) {
  ...
  for (let index = 0; index < results.length; index += 1) {
    ...
    const row = page
      .getByRole("row")
      .filter({ hasText: result.nickname })
      .first();
    const input = row.locator('input[placeholder="M:SS.mm"]').first();
    await ((await input.count()) ? input : inputs.nth(index)).fill(
      msToMSS(result.timeMs),
    );
  }

  await page
    .getByRole("button", { name: /Submit & Deduct Lives|送信＆ライフ減算/ })
    .click();
  const dialog = page
    .getByRole("dialog")
    .filter({
  ...
  const responsePromise = page.waitForResponse(
    (response) =>
      response.url().includes(`/api/tournaments/${tournamentId}/ta/phases`) &&
      response.request().method() === "POST",
    { timeout: 60_000 },
  );
  await dialog
    .getByRole("button", { name: /Confirm results|結果を確定/ })
    .click();
  const response = await responsePromise;
  pass(
    response.status() === 200,
    `UI submit_results failed (${response.status()})`,
  );
}
...
async function setup(adminPage) {
  ...
  handicapByPlayerId = new Map(
    players.map((player, index) => [player.id, handicaps[index]]),
  );
  ...
  pass(
    started.status === 201,
    `direct Phase 3 start failed (${started.status})`,
  );
  entries = started.body?.data?.entries ?? [];
  pass(
    entries.length === 6,
    `expected 6 Phase 3 entries, got ${entries.length}`,
  );
}
...
async function tcModeAndDirectStart(adminPage) {
  try {
    ...
    pass(
      phaseData?.phase3Rules?.initialLives === 10,
      "phase API initial lives mismatch",
    );
    pass(
      (phaseData?.phase3Rules?.lifeResetThresholds ?? []).length === 0,
      "BR unexpectedly has reset thresholds",
    );
    pass(
      (phaseData?.entries ?? []).every((entry) => entry.lives === 10),
      "Phase 3 entries did not start at 10 lives",
    );
    pass(
      (phaseData?.entries ?? []).every(
        (entry) =>
          entry.taHandicapSeconds === handicapByPlayerId.get(entry.playerId),
      ),
      "Phase 3 handicap snapshots differ from selected values",
    );

    const locked = await apiUpdateTournament(adminPage, tournamentId, {
      taBattleRoyaleMode: false,
    });
    pass(
      locked.s === 409 && locked.b?.code === "TA_MODE_LOCKED",
      `mode lock returned ${locked.s}/${locked.b?.code}`,
    );

    await nav(adminPage, `/tournaments/${tournamentId}/ta`);
    await adminPage.waitForURL(
      new RegExp(`/tournaments/${tournamentId}/ta/finals(?:$|\\?)`),
      { timeout: 15_000 },
    );
    pass(
      (await adminPage
        .locator(`a[href="/tournaments/${tournamentId}/bm"]`)
        .count()) === 0,
      "BM tab is visible",
    );
    pass(
      (await adminPage
        .locator(`a[href="/tournaments/${tournamentId}/mr"]`)
        .count()) === 0,
      "MR tab is visible",
    );
    pass(
      (await adminPage
        .locator(`a[href="/tournaments/${tournamentId}/gp"]`)
        .count()) === 0,
      "GP tab is visible",
    );
    pass(
      (await adminPage
        .locator(`a[href="/tournaments/${tournamentId}/overall-ranking"]`)
        .count()) === 0,
...
async function tcAdjustedRoundAndUi(adminPage) {
  try {
    ...
    const byPlayer = new Map(
      phaseEntries.map((entry) => [entry.playerId, entry]),
    );
    pass(
      byPlayer.get(entries[0].playerId)?.lives === 9,
      "0-second handicap player should lose a life",
    );
    pass(
      byPlayer.get(entries[1].playerId)?.lives === 10,
      "-1-second handicap player should be safe",
    );
    pass(
      byPlayer.get(entries[2].playerId)?.lives === 10,
      "-3-second handicap player should be safe",
    );
    pass(
      byPlayer.get(entries[3].playerId)?.lives === 10,
      "-5-second handicap player should be safe",
    );
    pass(
      byPlayer.get(entries[4].playerId)?.lives === 9,
      "second -3-second handicap player should lose a life",
    );
    pass(
      byPlayer.get(entries[5].playerId)?.lives === 9,
      "second -1-second handicap player should lose a life",
    );

    const storedRound = (phase.b?.data?.rounds ?? []).find(
      (round) => round.roundNumber === roundNumber,
    );
    const lastResult = storedRound?.results?.find(
      (result) => result.playerId === entries[3].playerId,
    );
      ...
      const update = await apiJson(
        ...
        `/api/tournaments/${tournamentId}/ta`,
        {
          ...
          body: {
            entryId: entry.id,
            action: "update_lives",
            livesDelta: 1 - current,
          },
        },
      );
      pass(
        update.status === 200,
        `failed to prepare one-life state for ${entry.playerId}`,
      );
    }
    ...
    pass(
      simultaneousStart.s === 200,
      `simultaneous elimination round failed to start (${simultaneousStart.s})`,
    );
    const simultaneousSubmit = await apiPostTaPhase(adminPage, tournamentId, {
      ...
      results: entries.map((entry, index) => ({
        playerId: entry.playerId,
        timeMs: rawTimes[index] + 10_000,
      })),
    });
    pass(
      simultaneousSubmit.s === 200,
      `simultaneous elimination submit failed (${simultaneousSubmit.s})`,
    );
    const afterSimultaneous = await apiFetchTaPhase(
      adminPage,
      tournamentId,
      "phase3",
    );
    ...
    const afterByPlayer = new Map(
      afterEntries.map((entry) => [entry.playerId, entry]),
    );
    pass(
    ...
    pass(
      afterEntries.every(
        (entry) => entry.lives > 0 || entry.eliminated === true,
      ),
    ...
    pass(
      /TA Battle Royale|TAバトルロワイヤル/.test(body),
      "BR mode badge not visible",
    );
    pass(
      body.includes("♥ 10/10") && /Eliminated|失格/.test(body),
      "compact lives/elimination state not visible",
    );

    await uiPhaseStartRound(adminPage, tournamentId, "phase3");
    const activePhase = await apiFetchTaPhase(
      adminPage,
      tournamentId,
      "phase3",
    );
    const active = (activePhase.b?.data?.entries ?? []).filter(
...
async function tcArchiveParity(adminPage) {
  try {
    const activated = await apiUpdateTournament(adminPage, tournamentId, {
      status: "active",
    });
    pass(activated.s === 200, `activation failed (${activated.s})`);
    const completed = await apiUpdateTournament(adminPage, tournamentId, {
      status: "completed",
      publicModes: ["ta"],
    });
    pass(completed.s === 200, `completion failed (${completed.s})`);

    const archiveResponse = await apiJson(
      adminPage,
    ...
    pass(
      ...
      `archive GET failed (${archiveResponse.status})`,
    );
    pass(archive?.schemaVersion === 2, "archive schema is not v2");
    pass(
      archive?.modes?.ta?.rules?.mode === "battle_royale",
      "archive mode mismatch",
    );
    pass(
      archive?.modes?.ta?.rules?.initialLives === 10,
      "archive rules mismatch",
    );
    pass(
      (archive?.modes?.ta?.phaseRounds ?? []).length >= 2,
      "archive round history missing",
    );

    await apiDeleteTournament(adminPage, tournamentId);
    const archivedPhase = await apiFetchTaPhase(
      adminPage,
      tournamentId,
      "phase3",
    );
    pass(
      archivedPhase.s === 200 && archivedPhase.b?.data?.archived === true,
      "phase API did not fall back to archive",
    );
    pass(
      archivedPhase.b?.data?.taMode === "battle_royale",
      "archived phase API mode mismatch",
    );
    pass(
      archivedPhase.b?.data?.phase3Rules?.initialLives === 10,
      "archived phase rules mismatch",
    );
    pass(
      (archivedPhase.b?.data?.rounds ?? []).length >= 2,
      "archived phase history missing",
    );
    tournamentId = null;
...
function getSuite() {
  return {
    ...
    afterAll: async (adminPage) => {
      ...
      await Promise.all(
        players.map((player) =>
          apiDeletePlayer(adminPage, player.id).catch(() => {}),
        ),
      );
      tournamentId = null;
      players = [];
