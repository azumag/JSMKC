...
function cdmE2eStructuralFailures(zipBytes) {
  ...
  if (!parts['xl/tables/table1.xml'])
    failures.push('xl/tables/table1.xml missing (Registration/Tracks/Cups tables dropped)');
  if (!parts['xl/richData/rdrichvalue.xml'])
    failures.push('xl/richData/rdrichvalue.xml missing (flag rich-data dropped)');
...
function cdmE2eFinalsSlotMatches(state) {
  return [...(state?.playoffMatches || []), ...(state?.matches || [])]
...
function assertCdmE2eGeneratorResults(results) {
  ...
  throw new Error(
    `CDM finals fixture generation failed: ${failures
      .map(({ mode, result }) => `${mode} ${cdmE2eGenerationResultDetail(result)}`)
      .join('; ')}`,
  );
}
...
async function generateCdmE2eMissingFinals(page, tournamentId, missingModes, api = cdmE2eFinalsApi) {
  ...
  const results = await Promise.all(generators.map(async ({ mode, promise }) => ({ mode, result: await promise })));
...
async function pageFetchJson(p, url, options = {}, timeoutMs = 30000) {
  ...
  return p.evaluate(
    async ({ requestUrl, requestOptions, requestTimeoutMs }) => {
...
async function main() {
  ...
  progressWatchdog = createProgressWatchdog('tc-all', envMs('E2E_PROGRESS_TIMEOUT_MS', 90 * 60 * 1000), () =>
  ...
  try {
    ...
    try {
      ...
      console.log(
        `[tc-all] shared all-mode tournament ready: ${TID} (${sharedSetup.playerIds.length} players, shared fixture)`,
      );
    } catch (err) {
      setupAllModesError = err instanceof Error ? err.message : String(err);
      TID = sharedFixture.normalTournament.id;
      const jsHint = setupJsErrors.length > 0 ? ` JS errors: ${setupJsErrors.slice(0, 3).join('; ')}` : '';
      const consoleHint =
        setupConsoleWarnings.length > 0 ? ` Console warnings: ${setupConsoleWarnings.slice(0, 3).join('; ')}` : '';
      console.error(
        `[tc-all] setupAllModes failed (${setupAllModesError.slice(0, 160)}); TC-401/TC-402 will be recorded as FAIL and the run will continue against ${TID}.${jsHint}${consoleHint}`,
      );
    } finally {
    ...
    const tc939CleanClasses = tc939TabClasses.every(
      (className) => className.trim() === className && !/\s{2,}/.test(className),
    );
    ...
    for (const { code, expected } of [
      ...
      const hasRecoveryLinks =
        (await page.locator('a[href="/auth/signin"]').count()) > 0 && (await page.locator('a[href="/"]').count()) > 0;
    ...
    log(
      'TC-2070B',
      vitalsStatus === 204 && t.includes('SMKC') ? 'PASS' : 'FAIL',
    ...
    log(
      'TC-008F',
      jpFlags > 0 && badFlags === 0 ? 'PASS' : 'FAIL',
      jpFlags > 0
        ? badFlags
          ? `${badFlags} malformed flag src`
          : `${jpFlags} JP flag(s)`
        : 'no /flags/jp.svg rendered',
    );
    ...
    try {
      await nav(page, '/players');
      await page
        .getByRole('button', { name: /^(Add Player|プレイヤー追加)$/ })
        .first()
        .click();
      const tc2997Dialog = page
        .getByRole('dialog')
        .filter({ has: page.locator('#nickname') })
        .first();
      ...
      const submitPromise = page.waitForResponse(
        (res) => res.url().includes('/api/players') && res.request().method() === 'POST',
        { timeout: 30000 },
      );
      ...
      await page
        .getByRole('button', { name: /I've Saved It|保存しました/ })
        .click({ timeout: 5000 })
        .catch(() => {});
      await page.waitForTimeout(1500);
      const rosterRow = page.getByRole('row', { name: new RegExp(tc2997Nickname) });
      const rosterFlag = await rosterRow
        .locator('img[src="/flags/de.svg"]')
        .count()
        .catch(() => 0);
      ...
      log(
        ...
        submitResponse.status() !== 201
          ? `POST status=${submitResponse.status()}`
          : triggerFlagAfterSelect === 0
            ? 'trigger did not show DE flag after picking Germany'
            : rosterFlag === 0
              ? 'roster row did not show DE flag after save'
              : '',
      );
    } catch (err) {
    ...
    const miss = ['content-security-policy', 'x-frame-options', 'x-content-type-options', 'referrer-policy'].filter(
      (h) => !hdrs[h],
    );
    ...
    const tc108HasShape = [tc108P1, tc108P2, tc108Clamp].every(
      (r) => r?.success === true && Array.isArray(r?.data) && r?.meta,
    );
    ...
    log(
      'TC-108',
      tc108HasShape && tc108MetaOk && tc108DistinctOk && tc108ClampOk && tc108UiOk ? 'PASS' : 'FAIL',
      !tc108HasShape
        ? 'API shape mismatch'
        : !tc108MetaOk
          ? `Unexpected meta page=${tc108Meta.page} limit=${tc108Meta.limit} total=${tc108Meta.total} totalPages=${tc108Meta.totalPages}`
          : !tc108DistinctOk
            ? 'Page 1 and page 2 returned same IDs despite total>10'
            : !tc108ClampOk
              ? `Limit clamp failed: meta.limit=${tc108ClampLimit}`
              : !tc108UiOk
                ? 'Pager UI not visible despite total>50'
                : '',
    );
    ...
    try {
      ...
      const tournamentsPageLoaded = t.includes('Tournaments') || t.includes('トーナメント') || t.includes('大会');
      const knownTournamentExists =
        knownTournament.s === 200 &&
        (knownTournament.b?.data?.name || knownTournament.b?.name || '') === tc201TournamentName;
      log(
        'TC-202',
        tournamentsPageLoaded && knownTournamentExists ? 'PASS' : 'FAIL',
        !tournamentsPageLoaded
          ? 'Tournaments page did not render'
          : !knownTournamentExists
            ? 'Created tournament not found by API'
            : '',
      );
    } catch (err) {
    ...
    // tournament data; the hidden badge should appear/disappear without a page reload.
    {
      ...
      try {
        ...
        const pass =
          switchExists &&
          hasBadgeBefore &&
          !hasBadgeAfterPublish &&
          hasBadgeAfterUnpublish &&
          overallSwitchExists &&
          hasOverallBadgeBefore &&
          !hasOverallBadgeAfterPublish &&
          hasOverallBadgeAfterUnpublish;
        log(
          'TC-823',
          pass ? 'PASS' : 'FAIL',
          !switchExists
            ? 'Publish switch not found on BM page'
            : !hasBadgeBefore
              ? 'Hidden badge not shown before toggle'
              : hasBadgeAfterPublish
                ? 'Hidden badge still shown after toggle to published (event/re-fetch missing)'
                : !hasBadgeAfterUnpublish
                  ? 'Hidden badge did not reappear after re-toggle to unpublished'
                  : !overallSwitchExists
                    ? 'Publish switch not found on overall ranking page'
                    : !hasOverallBadgeBefore
                      ? 'Overall hidden badge not shown before toggle'
                      : hasOverallBadgeAfterPublish
                        ? 'Overall hidden badge still shown after toggle to published (event/re-fetch missing)'
                        : !hasOverallBadgeAfterUnpublish
                          ? 'Overall hidden badge did not reappear after re-toggle to unpublished'
                          : '',
        );
      } catch (err) {
    ...
    if (setupAllModesError) {
      ...
      try {
        ...
        log(
          'TC-401',
          taOk && bmOk && mrOk && gpOk ? 'PASS' : 'FAIL',
          !taOk
            ? `TA entries=${taEntries.length}`
            : !bmOk
              ? `BM matches=${bmMatches.length} completed=${bmMatches.filter((m) => m.completed).length}`
              : !mrOk
                ? `MR matches=${mrMatches.length} completed=${mrMatches.filter((m) => m.completed).length}`
                : !gpOk
                  ? `GP matches=${gpMatches.length} completed=${gpMatches.filter((m) => m.completed).length}`
                  : '',
        );
      } catch (err) {
    ...
    if (setupAllModesError) {
      ...
      try {
        ...
        const topBreakdown = topOverallRanking
          ...
          : null;
        const hasAllModes = rankings.some(
          (r) =>
        ...
        const stored = await pageFetchJson(
          page,
        ...
        log(
          'TC-402',
          calc.s === 200 && ranksOk && hasAllModes && totalsOk && persistedOk && displayOk ? 'PASS' : 'FAIL',
          calc.s !== 200
            ? `POST returned ${calc.s}: ${JSON.stringify(calc.b).slice(0, 200)}`
            : !ranksOk
              ? `rankings=${rankings.length} topRank=${topOverallRanking?.overallRank}`
              : !hasAllModes
                ? 'No ranking row has points from all 4 qualification modes'
                : !totalsOk
                  ? `top total=${topOverallRanking?.totalPoints} breakdown=${topBreakdown}`
                  : !persistedOk
                    ? 'GET did not return persisted calculation'
                    : !displayOk
                      ? `Overall page missing top ranking ${topOverallRanking?.playerNickname}`
                      : '',
        );
      } catch (err) {
    ...
    if (pid) {
      try {
        ...
        const passwordDialogAppeared = await page
          .getByRole('dialog')
          .filter({
        ...
        const pass =
          status === 409 && dialogStillOpen && hasErrorMessage && !passwordDialogAppeared && nicknameCount === 1;
    ...
    if (pid && playerTempPassword) {
      ...
      try {
        ...
        if (hasParticipantLink) {
          ...
          await playerPage.waitForURL((url) => url.pathname === `/tournaments/${TID}/gp/participant`, {
            timeout: 15000,
          });
        ...
        const hasLoggedInState =
          playerText.includes('プレイヤーとしてログイン中') || playerText.includes('Logged in as player');
        const hasParticipantEmptyState =
          playerText.includes('保留中の試合はありません') || playerText.includes('No Pending Matches');
        const showsLoginPrompt =
          playerText.includes('プレイヤーログインが必要です') || playerText.includes('Player Login Required');
    ...
    if (pid && playerTempPassword) {
      ...
      try {
        ...
        gpTournamentId = await uiCreateTournament(page, `E2E GP Score Entry ${Date.now()}`, {
        ...
        log(
          'TC-311',
          scorePersisted ? 'PASS' : 'FAIL',
          scorePersisted
            ? ''
            : !reportedMatch
              ? 'No matching match found in API response'
    ...
    if (pid && playerTempPassword) {
      ...
      try {
        taTournamentId = await uiCreateTournament(page, `E2E TA Knockout Lock ${Date.now()}`, {
          dualReportEnabled: false,
        });
        ...
        await uiSetupTaPlayers(page, taTournamentId, [{ id: pid, name: playerName, nickname: nick, seeding: 17 }]);
        ...
        log(
          'TC-312',
          warningVisible && submitDisabled && inputDisabled ? 'PASS' : 'FAIL',
          !warningVisible
            ? 'No knockout lock warning'
            : !submitDisabled
              ? 'Submit button still enabled'
              : !inputDisabled
                ? 'Time input still enabled'
                : '',
        );
        await playerBrowser.close();
        playerBrowser = null;
    ...
    if (pid) {
      ...
      try {
        ...
        const toastVisible = await page
          ...
          .filter({
            hasText:
              /本線開始後は、予選へのプレイヤー追加はできません。|Players cannot be added to qualification after the knockout stage starts./,
          })
          .count()
          .then((count) => count > 0);
        const dialogOpened = await page
          .getByText(/Setup Time Trial Players|Edit Time Trial Players|タイムアタック プレイヤー(設定|編集)/)
          .count()
          .then((count) => count > 0);

        log(
          'TC-313',
          ariaDisabled === 'true' && toastVisible && !dialogOpened ? 'PASS' : 'FAIL',
          ariaDisabled !== 'true'
    ...
    if (pid) {
      ...
      try {
        ...
        taTournamentId = await uiCreateTournament(page, `E2E TA Finals Undo ${Date.now()}`, {
          dualReportEnabled: false,
        });
        await uiActivateTournament(page, taTournamentId);
        ...
        const cancelButtonVisible = await page
          .getByRole('button', { name: /ラウンドキャンセル|Cancel Round/ })
          .count()
          .then((count) => count > 0);
    ...
    if (pid) {
      ...
      try {
        ...
        await uiSetupTaPlayers(page, taTournamentId, [{ id: pid, name: playerName, nickname: nick, seeding: 3 }]);
        ...
        log(
          'TC-317',
          step1 && step2 && step3 && step4 ? 'PASS' : 'FAIL',
          !step1
            ? 'Initial seeding not set on creation'
            : !step2
              ? 'update_seeding PUT failed'
              : !step3
                ? `Seeding not persisted in GET (got ${entry?.seeding})`
                : !step4
                  ? 'Failed to clear seeding to null'
                  : '',
        );
      } catch (err) {
      ...
      try {
        ...
        log(
          ...
          !step1
            ? 'set_partner failed'
            : !step2
              ? `Bidirectional partner not set (e1.partnerId=${e1?.partnerId}, e2.partnerId=${e2?.partnerId})`
              : !step3
                ? 'Partner could not edit paired player time'
                : !step4
                  ? `Time not persisted (got ${updatedEntry?.times?.MC1})`
                  : '',
        );
      } catch (err) {
    ...
    if (pid) {
      ...
      try {
        ...
        const selfEditResult = await apiTaParticipantEditTime(page, taTournamentId, entry1.id, 'MC1', '1:00.00');
        const getResult = await apiFetchTa(page, taTournamentId);
        ...
        log(
          ...
          step1 && step2 && step3 && step4 ? 'PASS' : 'FAIL',
          !step1
            ? `taPlayerSelfEdit not false (${getResult.b?.data?.taPlayerSelfEdit})`
            : !step2
              ? 'Admin edit failed'
              : !step3
                ? 'Toggle PUT failed'
                : !step4
                  ? 'taPlayerSelfEdit not toggled back to true'
                  : '',
        );
      } catch (err) {
    ...
    // players there is exactly 1 non-BYE match — enough to verify the TV select.
    {
      ...
      try {
        ...
        const bmSetup518 = await page.evaluate(
          async ([tid, p1, p2]) => {
        ...
        await page
          .waitForFunction(() => document.querySelectorAll('select.w-14').length > 0, null, { timeout: 40000 })
          .catch(() => {});
        ...
        log(
          'TC-518',
          hasSelect && optionsOk && assignOk ? 'PASS' : 'FAIL',
          !hasSelect
            ? 'No TV select found on BM qualification page'
            : !optionsOk
              ? `TV options missing 1-4 (got ${options.join(',')})`
              : !assignOk
                ? 'TV 4 assignment did not persist'
                : '',
        );
      } catch (err) {
    ...
    log(
      'TC-304',
      mrSharedOk ? 'PASS' : 'FAIL',
      mrSharedOk
        ? ''
        : mrPageLoaded
          ? 'Shared MR tournament did not render configured groups'
          : 'MR page failed to load',
    );
    ...
    if ((await editBtn305.count()) > 0) {
      ...
      if ((await saveBtn305.count()) > 0) {
        ...
        log('TC-305', 'SKIP', 'No update button');
      }
    } else {
      log('TC-305', 'SKIP', 'No edit button');
    }
    ...
    // TC-1075: BM group setup dialog keeps the locked 2-group seeding path stable
    {
      ...
      try {
        ...
        const seedsMatch = Object.entries(expectedGroups).every(
          ([seed, group]) => groupBySeed.get(Number(seed)) === group,
        );
        const balanced = ['A', 'B'].every((group) => counts[group] === 4);
        log(
          'TC-1075',
          seedsMatch && balanced ? 'PASS' : 'FAIL',
          !seedsMatch
            ? `groups=${JSON.stringify(Object.fromEntries(groupBySeed))}`
            : !balanced
      ...
      try {
        ...
        if (!tc315TournamentId) {
          ...
          if (players315.length < 3) {
            ...
            const setup315 = await page.evaluate(
              async ([u, d]) => {
                const r = await fetch(u, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
            ...
            log(
              'TC-315',
              postOk && hasByeMatch && noBreakInPlayers ? 'PASS' : 'FAIL',
              !postOk
                ? `POST returned ${setup315.s}`
                : !hasByeMatch
                  ? 'No BYE match found'
                  : !noBreakInPlayers
                    ? '__BREAK__ appeared in player list'
                    : '',
            );
          }
        }
          ...
          if (bmSetupOk) {
            {
              ...
              const hasTieWarnBefore =
                text1.includes('同順位が検出されました') || text1.includes('Tied ranks detected');
              ...
              if (!hasGroups) {
                ...
                if (groupAMatches.length === 0) {
                  ...
                  log(
                    'TC-316',
                    'PASS',
                    '0-match warning correctly suppressed (no group matches available for phase 2)',
                  );
                } else {
                  // Enter a 2-2 draw (score1=2 score2=2 → both players tied at mp=1)
                  const enter316 = await page.evaluate(
                    async ([u, d]) => {
                      const r = await fetch(u, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
                  ...
                  if (enter316.s !== 200) {
                    log(
                      'TC-316',
                      'PASS',
                      `0-match warning suppressed (tie entry returned ${enter316.s}, cannot verify phase 2)`,
                    );
                  } else {
                    await nav(page, `/tournaments/${tc316TournamentId}/bm`);
                    const text2 = await vis(page);
                    const hasTieWarnAfter =
                      text2.includes('同順位が検出されました') || text2.includes('Tied ranks detected');
                    log(
                      'TC-316',
                      hasTieWarnAfter ? 'PASS' : 'FAIL',
    ...
    // check is skipped to avoid a false-negative — the check is only meaningful when matches exist.
    {
      ...
      for (const m of ['bm', 'mr', 'gp']) {
        ...
        if (m === 'bm') {
          ...
          if (!hasMatchesTab) {
            log('TC-320', 'SKIP', 'BM Matches tab absent (no BM setup for TID)');
            continue;
          }
          ...
          const hasRowScoreEntry = await page
            .locator('tbody a:has-text("Score Entry"), tbody a:has-text("スコア入力")')
            .count();
    ...
    //   3. Shows "in progress" message for incomplete matches
    {
      ...
      try {
        ...
        const showsInProgress = matchText.includes('in progress') || matchText.includes('進行中');

        log(
          'TC-321',
          showsPlayers && noScoreEntryForm && showsInProgress ? 'PASS' : 'FAIL',
          !showsPlayers
            ? 'Match page does not show player info'
            : !noScoreEntryForm
              ? 'Match page still has score entry form elements'
              : !showsInProgress
                ? 'Match page does not show "in progress" message'
                : '',
        );
      } catch (err) {
      ...
      try {
        ...
        for (const match of nonByeMatches) {
          ...
          const put = await page.evaluate(
            ...
            [`/api/tournaments/${tc323TournamentId}/bm`, { matchId: match.id, score1: 2, score2: 2 }],
          );
        ...
        const bannerBefore =
          (await page.locator('text=同順位が検出されました').count()) +
        ...
        log(
          ...
          !hasBannerBefore
            ? 'Tie warning banner never appeared (expected tie from 2-2 draws)'
            : hasBannerAfter
              ? 'Tie warning banner still visible after setting rankOverride on N-1 players'
              : '',
        );
      } catch (err) {
    ...
    // Uses the admin persistent session from the profile
    {
      try {
        ...
        log('TC-325', pass ? 'PASS' : 'FAIL', !pass ? 'Profile page neither shows user info nor login redirect' : '');
      } catch (err) {
    ...
    if (TID) {
      try {
        ...
        const hasBom =
          exportResp.bomBytes[0] === 0xef && exportResp.bomBytes[1] === 0xbb && exportResp.bomBytes[2] === 0xbf;
        log(
          'TC-326',
          exportResp.status === 200 && hasCsvContent && csvNotEmpty && hasBom ? 'PASS' : 'FAIL',
          exportResp.status !== 200
            ? `Export returned ${exportResp.status}`
            : !hasCsvContent
              ? 'No CSV content-type'
              : !csvNotEmpty
                ? 'CSV is empty'
    ...
    // TC-327: Session status API returns session information
    {
      try {
        ...
        const isAuthenticated =
          sessResp.body?.success === true && typeof sessResp.body?.data?.authenticated === 'boolean';
        const isUnauthenticated = sessResp.body?.success === false && sessResp.body?.requiresAuth === true;
        ...
        log(
          ...
          tc327Pass
            ? ''
            : sessResp.status !== 200
              ? `Session status returned ${sessResp.status}`
      ...
      try {
        ...
        log(
          ...
          adminResp.status !== 200
            ? `Admin got ${adminResp.status}`
            : !hasShape
              ? 'Response shape invalid'
              : !anonBlocked
                ? `Anon got ${anonStatus} (expected 401/403)`
                : '',
        );
      } catch (err) {
    ...
    if (pid) {
      ...
      try {
        ...
        await uiSetupTaPlayers(page, tc331TournamentId, [{ id: pid, name: playerName, nickname: nick }]);
        ...
        log(
          ...
          getResp.status !== 200
            ? `Got ${getResp.status}`
            : !hasShape
              ? 'Response missing player, tournament, or version fields'
              : '',
        );
      } catch (err) {
    ...
    if (pid) {
      ...
      try {
        ...
        await uiSetupTaPlayers(page, tc332TournamentId, [{ id: pid, name: playerName, nickname: nick }]);
        ...
        const readResp = await page.evaluate(
          async ([tid, eid]) => {
            const r = await fetch(`/api/tournaments/${tid}/tt/entries/${eid}`);
            return { status: r.status, body: await r.json().catch(() => ({})) };
          },
        ...
        const conflictResp = await page.evaluate(
          async ([tid, eid, staleVersion]) => {
            const r = await fetch(`/api/tournaments/${tid}/tt/entries/${eid}`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
        ...
        log(
          'TC-332',
          conflictResp.status === 409 ? 'PASS' : 'FAIL',
    ...
    if (pid) {
      ...
      try {
        ...
        await uiSetupTaPlayers(page, tc342TournamentId, [{ id: pid, name: playerName, nickname: nick }]);
        ...
        const partialResp = await page.evaluate(
          async ([tid, eid, ver]) => {
            const r = await fetch(`/api/tournaments/${tid}/tt/entries/${eid}`, {
              method: 'PUT',
              headers: { 'Content-Type': 'application/json' },
        ...
        const hasErrorMsg = typeof partialResp.body?.error === 'string' && partialResp.body.error.includes('20');
        log(
          'TC-342',
          is400 && hasErrorMsg ? 'PASS' : 'FAIL',
          !is400
            ? `Expected 400 for partial times, got ${partialResp.status}`
            : !hasErrorMsg
              ? `Missing/wrong error message: ${partialResp.body?.error}`
              : '',
        );
      } catch (err) {
    ...
    if (pid) {
      ...
      try {
        ...
        await uiSetupTaPlayers(page, tc343TournamentId, [{ id: pid, name: playerName, nickname: nick }]);
        ...
        log(
          'TC-343',
          readResp.status === 200 && hasLastCourse && hasLastTime ? 'PASS' : 'FAIL',
          readResp.status !== 200
            ? `GET failed: ${readResp.status}`
            : !hasLastCourse
              ? `lastRecordedCourse=${entryData?.lastRecordedCourse} (expected 'RR')`
              : !hasLastTime
                ? `lastRecordedTime not set (got ${entryData?.lastRecordedTime})`
                : '',
        );
      } catch (err) {
    ...
    try {
      ...
      log(
        ...
        anonStatus !== 403
          ? `Anon got ${anonStatus} (expected 403)`
          : adminResp.status !== 200
            ? `Admin got ${adminResp.status}`
            : '',
      );
    ...
    try {
      ...
      let parsedDetail = {};
      try {
        parsedDetail = JSON.parse(anonDetail.body);
      } catch (_) {
        /* ignore */
      }
      const modesPublished =
        Array.isArray(parsedDetail?.data?.publicModes) && parsedDetail.data.publicModes.includes('ta');
      ...
      log(
        ...
        anonDetail.status !== 200
          ? `Anon detail got ${anonDetail.status}`
          : !modesPublished
            ? 'publicModes not updated in response'
            : !listStatus.found
              ? 'Tournament not in public list after publishing'
              : '',
      );
    } catch (err) {
    ...
    if (TID) {
      try {
        ...
        log(
          ...
          !hasPhaseStatus
            ? `Base response invalid (status=${baseResp.status}, phaseStatus=${typeof baseResp.body?.data?.phaseStatus})`
            : !hasPhaseData
              ? `Phase param response invalid (status=${phaseResp.status})`
              : invalidResp.status !== 400
                ? `Invalid phase param got ${invalidResp.status} (expected 400)`
                : '',
        );
      } catch (err) {
    ...
    try {
      ...
      log(
        ...
        !hasShape
          ? `Limit=1 response shape invalid (status=${limitResp.status}, data.length=${paginatedData?.data?.length}, meta=${JSON.stringify(meta)})`
          : !page2Ok
            ? `Page 2 response invalid (status=${page2Resp.status})`
            : '',
      );
    } catch (err) {
    ...
    try {
      ...
      log(
        ...
        !notLeaked
          ? 'Private tournament appeared in non-admin list (metadata leak)'
          : !adminList.found
            ? 'Admin could not see private tournament in list'
            : '',
      );
    } catch (err) {
    ...
    try {
      ...
      const bmOnlyAccepted =
        bmOnlyResp.status === 200 &&
      ...
      const bmOffNoCascade =
        bmOffResp.status === 200 &&
      ...
      const overallOnlyAccepted =
        overallOnlyResp.status === 200 &&
      ...
      log(
        'TC-339',
        bmOnlyAccepted && taAdded && bmOffNoCascade && overallOnlyAccepted && invalidRejected && appearsInList
          ? 'PASS'
          : 'FAIL',
        !bmOnlyAccepted
          ? `Independent BM publish failed (status=${bmOnlyResp.status}, modes=${JSON.stringify(bmOnlyResp.body?.data?.publicModes)})`
          : !taAdded
            ? `TA add did not preserve BM (modes=${JSON.stringify(taAddResp.body?.data?.publicModes)})`
            : !bmOffNoCascade
              ? `BM off cascaded into TA (modes=${JSON.stringify(bmOffResp.body?.data?.publicModes)})`
              : !overallOnlyAccepted
                ? `Overall publish failed (status=${overallOnlyResp.status}, modes=${JSON.stringify(overallOnlyResp.body?.data?.publicModes)})`
                : !invalidRejected
                  ? `Invalid modes accepted (foo=${invalidPut.status}, dupe=${dupePut.status}, expected 400)`
                  : !appearsInList
                    ? 'Published tournament not visible in non-admin list'
                    : '',
      );
    } catch (err) {
    ...
    try {
      ...
      log(
        ...
        !jumpRejected
          ? `draft→completed not rejected (status=${jump.status}, expected 400)`
          : !bogusRejected
            ? `unknown status not rejected (status=${bogus.status}, expected 400)`
            : !forwardOk
              ? `forward flow failed (start=${started.status}, complete=${completedResp.status})`
              : !buttonVisible
                ? 'Reopen button not visible on completed tournament'
                : !reopenedViaUi
                  ? 'Reopen click did not set status back to active'
                  : '',
      );
    } catch (err) {
    ...
    try {
      ...
      const badgeBeforePublish = await hiddenBadge()
        .isVisible()
        .catch(() => false);
      ...
      const badgeAfterPublish = await hiddenBadge()
        .isVisible()
        .catch(() => false);
      ...
      await page
        ...
        .catch(() => {});
      const badgeAfterUnpublish = await hiddenBadge()
        .isVisible()
        .catch(() => false);
      ...
      log(
        ...
        !badgeBeforePublish
          ? 'Unpublished badge not visible in tab before publish'
          : badgeAfterPublish
            ? 'Unpublished badge still visible after publish (re-render failed?)'
            : !badgeAfterUnpublish
              ? 'Unpublished badge did not reappear after unpublish'
              : '',
      );
    } catch (err) {
    ...
    if (pid && playerTempPassword) {
      ...
      try {
        ...
        log(
          ...
          !unauthBlocked
            ? `Unauthenticated should get 403, got ${unauthStatus}`
            : !playerAllowed
              ? `Player should get 200, got ${tc341PlayerResp.status} (${JSON.stringify(tc341PlayerResp.body)})`
              : '',
        );
      } catch (err) {
        log(
          'TC-341',
          'FAIL',
          err instanceof Error ? err.message : 'Authenticated player private tournament test failed',
        );
      } finally {
    ...
    // TC-344: noCamera flag — create player with noCamera=true, verify in API and UI, toggle via edit
    {
      ...
      try {
        ...
        log(
          'TC-344',
          noCameraTrue && updateOk && noCameraFalse ? 'PASS' : 'FAIL',
          !noCameraTrue
            ? `noCamera should be true after create, got ${getResp.b?.data?.noCamera}`
            : !updateOk
              ? `PUT noCamera=false failed (${putResp.s}), noCamera=${putResp.b?.data?.noCamera}`
              : !noCameraFalse
                ? `noCamera should be false after update, got ${getResp2.b?.data?.noCamera}`
                : '',
        );
      } catch (err) {
    ...
    // test is independent of the shared 28-player setup.
    {
      ...
      try {
        ...
        const bmSetup345 = await page.evaluate(
          async ([tid, p1, p2, p3, p4]) => {
            const r = await fetch(`/api/tournaments/${tid}/bm`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
        ...
        const patch1 = await page.evaluate(
          async ([tid, mid]) => {
            const r = await fetch(`/api/tournaments/${tid}/bm`, {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
        ...
        log(
          'TC-345',
          firstOk && dupRejected ? 'PASS' : 'FAIL',
          !firstOk
            ? `First TV1 assignment failed (${patch1.s})`
            : !dupRejected
              ? `Duplicate TV1 was accepted (${patch2.s}) — uniqueness guard missing`
              : '',
        );
      } catch (err) {
    ...
    // After bracket creation, all losers_r1 match slots are NULL, never a real
    // entrant. After one QF match completes and its loser is routed,
    ...
    // This test covers the API-level invariant; the component fix is covered by the type check.
    {
      ...
      try {
        ...
        const bmSetup346 = await page.evaluate(
          async ([tid, pids]) => {
            const r = await fetch(`/api/tournaments/${tid}/bm`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                players: [
                  { playerId: pids[0], group: 'A' },
                  { playerId: pids[1], group: 'A' },
                  { playerId: pids[2], group: 'A' },
                  { playerId: pids[3], group: 'A' },
                  { playerId: pids[4], group: 'B' },
                  { playerId: pids[5], group: 'B' },
                  { playerId: pids[6], group: 'B' },
                  { playerId: pids[7], group: 'B' },
                ],
              }),
            });
            return { s: r.status, b: await r.json().catch(() => ({})) };
        ...
        // Verify: all losers_r1 matches initially have no real-player placement.
        const losersR1 = allMatches346.filter((m) => m.round === 'losers_r1');
        const allPlaceholder = losersR1.length > 0 && losersR1.every((m) => m.player1Id == null && m.player2Id == null);
        ...
        log(
          ...
          !allPlaceholder
            ? `losers_r1 initial TBD check failed (${losersR1.length} matches, expected null slots)`
            : !partiallyFilled
              ? 'After QF completion, no losers_r1 match was partially filled (routing failed?)'
              : '',
        );
      } catch (err) {
    ...
    // TC-347: Export API — GET /api/tournaments/:id/export returns CSV with BOM
    {
      ...
      if (exportTid) {
        try {
          ...
          const isCsv =
            (resp.contentType || '').includes('text/csv') || (resp.contentType || '').includes('application/csv');
          ...
          log(
            'TC-347',
            resp.status === 200 && isCsv && hasContent && hasHeaderRow ? 'PASS' : 'FAIL',
            resp.status !== 200
              ? `HTTP ${resp.status}`
              : !isCsv
                ? `content-type: ${resp.contentType}`
                : !hasHeaderRow
                  ? 'no CSV header row found'
                  : '',
          );
        } catch (err) {
        ...
        try {
          ...
          const responsePromise = page.waitForResponse(
            (response) => {
              ...
              return (
                url.pathname === `/api/tournaments/${exportTid}/export` && url.searchParams.get('format') === 'cdm'
              );
            },
            { timeout: 60000 },
          );
          ...
          const [, apiResponse, download] = await Promise.all([cdmButton.click(), responsePromise, downloadPromise]);
                  ...
                  : !isXlsm
                    ...
                    : !hasXlsmAttachment
                      ...
                      : !hasDownloadedXlsmName
                        ...
                        : !hasDownloadPath
                          ? `download path unavailable${downloadPathError ? `: ${downloadPathError}` : ''}; ensure acceptDownloads=true on the persistent context`
                          : !hasBody
                            ? `workbook too small: ${bytes.length} bytes`
                            : !hasZipSignature
                              ? `unexpected XLSM signature: ${Array.from(bytes.slice(0, 4)).join(',')}`
                              : '',
          );
        } catch (err) {
        ...
        try {
          ...
          if (exportResp.status !== 200) {
            ...
            for (const { mode, sheetName, state } of modeStates) {
              ...
              for (const match of cdmE2eFinalsMatches(state)) {
                ...
                for (const slotIndex of [0, 1]) {
                  ...
                  if (!nicknames.includes(actualName)) {
                    failures.push(
                      `${mode} ${slotRound} name ${nameRef}: ${actualName} not in [${nicknames.join(', ')}]`,
                    );
                  }
                }
                ...
                if (match.completed) {
                  const expectedScores = [cdmE2eMatchScore(match, mode, 0), cdmE2eMatchScore(match, mode, 1)];
                  if (expectedScores.every((s) => s != null)) {
                    ...
                    if (!scoreMatch) {
                      failures.push(
                        `${mode} ${slotRound} score @${slot.blockStart + CDM_FINALS_BLOCK_SCORE_OFFSET},row ${slot.row}: expected {${expSorted.join(',')}}, got {${actualScores.map((v) => v ?? '<blank>').join(',')}}`,
                      );
                    }
                  }
                }
                ...
                for (const slotIndex of [0, 1]) {
                  ...
                  if (!Number.isInteger(seedVal) || seedVal < 1 || seedVal > 24) {
                    failures.push(
                      `${mode} ${slotRound} seed ${seedRef}: expected B-position 1..24, got ${seedVal ?? '<blank>'}`,
                    );
                  }
                }
              }
              ...
              // is empty there.)
              const allowedSeedNames = await cdmE2eQualificationNicknames(page, exportTid, mode.toLowerCase());
              for (const match of cdmE2eFinalsMatches(state)) {
            ...
            log(
              ...
              structuralFailures.length > 0
                ? `structural: ${structuralFailures.slice(0, 3).join('; ')}`
                : checked === 0
                  ? `No finals matches available for CDM coordinate verification (${readinessSummary})`
                  : missingModes.length > 0
                    ? `No finals cells checked for modes: ${missingModes.join(', ')} (${readinessSummary})`
                    : failures.length > 0
      ...
      if (exportTid) {
        ...
        try {
          ...
          log(
            'TC-359',
            hasExportFailure &&
              (alertText.includes('Forbidden') || alertText.includes('session expired') || alertText.includes('権限'))
              ? 'PASS'
              : 'FAIL',
            alertText,
          );
      ...
      if (exportTid) {
        ...
        try {
          ...
          const loadingVisible = await page
            .getByRole('button', { name: /Exporting|エクスポート中/i })
          ...
          await page
            .getByRole('button', { name: /CDM Export/i })
          ...
          log(
            ...
            !loadingVisible
              ? 'Exporting label was not visible'
              : !isBusy
                ? 'aria-busy was not true while exporting'
                : !isDisabled
                  ? 'button was not disabled while exporting'
                  : requestCount !== 1
                    ? `expected 1 export request, got ${requestCount}`
                    : '',
          );
        } catch (err) {
        ...
        try {
          ...
          const retryResponsePromise = page.waitForResponse(
            (response) => {
              const url = new URL(response.url());
              return (
                url.pathname === `/api/tournaments/${exportTid}/export` &&
          ...
          log(
            'TC-361',
            hasExportFailure && reenabledAfterError && !busyAfterError && requestCount === 2 ? 'PASS' : 'FAIL',
            !hasExportFailure
              ? firstAlertText
              : !reenabledAfterError
                ? 'button stayed disabled after export error'
                : busyAfterError
                  ? 'aria-busy stayed true after export error'
                  : requestCount !== 2
                    ? `expected retry request count 2, got ${requestCount}`
                    : '',
          );
        } catch (err) {
    ...
    // TC-348: Character stats API — admin gets stats shape, non-admin gets 403
    {
      ...
      if (statsPid) {
        try {
          ...
          const hasShape = adminResp.status === 200 && adminResp.body !== null && typeof adminResp.body === 'object';
          ...
          log(
            'TC-348',
            hasShape && unauthedStatus === 401 ? 'PASS' : 'FAIL',
            !hasShape
              ? `admin: HTTP ${adminResp.status}`
              : unauthedStatus !== 401
                ? `unauthed: expected 401 got ${unauthedStatus}`
                : '',
          );
        } catch (err) {
    ...
    // TC-349: Responsive — BM/MR/GP qualification pages load without JS errors on narrow viewport
    {
      ...
      if (respTid) {
        ...
        try {
          ...
          const modePages = [`/tournaments/${respTid}/bm`, `/tournaments/${respTid}/mr`, `/tournaments/${respTid}/gp`];
          let allOk = true;
          for (const path of modePages) {
            await nav(page, path);
            if (jsErrors.length > 0 || serverErrors.length > 0) {
              allOk = false;
              break;
            }
    ...
    // the "TBD" placeholder text for unfilled loser-bracket slots.
    {
      ...
      try {
        ...
        await page
          .waitForFunction(() => document.querySelector('[data-testid="bracket-match-card"]') !== null, null, {
            timeout: 25000,
          })
          .catch(() => {});
        ...
        log(
          ...
          !hasTbd
            ? 'Bracket page did not show TBD for empty loser slots'
            : !routed350
              ? 'Routed player nickname not visible in bracket UI after QF completion'
              : '',
        );
      } catch (err) {
    ...
    // Confirming BM qual must NOT lock MR/GP scores; each mode has its own flag.
    {
      ...
      try {
        ...
        let mrScoreStatus = 0;
        if (mrMatchId) {
          mrScoreStatus = await page.evaluate(
            async ({ tid, mid }) => {
              const r = await fetch(`/api/tournaments/${tid}/mr`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    // caps each round-trip at 96 params and limits D1 retry tail from ~6s to ~2s.
    {
      ...
      try {
        ...
        const bmSetup353 = await page.evaluate(
          async ([tid, pids]) => {
        ...
        const mrSetup353 = await page.evaluate(
          async ([tid, pids]) => {
            const r = await fetch(`/api/tournaments/${tid}/mr`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
    ...
    // TC-354: Broadcast API — GET returns shape; admin PUT updates; non-admin PUT blocked
    {
      ...
      try {
        ...
        const clearOk =
          putClear.s === 200 && putClear.data.matchLabel === null && hasNoInternalBroadcastFields(putClear.data);
        ...
        const uiInvalidScoreVisible = await page
          .getByText('1P 点数、2P 点数は0以上の整数で入力してください。')
          .isVisible()
          .catch(() => false);
        const uiInvalidScoreStyled = await page
          .locator('#broadcast-player1-wins')
          .evaluate((el) => el.getAttribute('aria-invalid') === 'true' && el.className.includes('border-destructive'))
          .catch(() => false);
        ...
        const ok =
          hasShape &&
          putOk &&
          putResponsePublicShape &&
          getOk &&
          clearOk &&
          clearWinsOk &&
          invalidScoresBlocked &&
          uiInvalidScoreVisible &&
          uiInvalidScoreStyled &&
            ...
            .waitForFunction(
              (titles) => {
                ...
                const hasModeContent =
                  /(Qualification|Qualifying|予選|Entries|Players|参加者|Matches|試合|Ranking|順位)/.test(body);
                const main = document.querySelector('main');
                const hasStructuredContent =
                  main?.querySelector('table, [role="table"], button, a, input, select') !== null;
                return (
                  !hasError &&
                  Boolean(main) &&
                  body.trim().length > 20 &&
                  (hasExpectedTitle || hasModeContent || hasStructuredContent)
                );
              },
              expectedTitles[mode],
              { timeout: 15000 },
      ...
      for (const mode of modes357) {
        try {
          ...
          const hasExpectedTitle = await page.evaluate((titles) => {
            ...
            const normalizedHeadings = headings.map((heading) => (heading.textContent || '').trim()).filter(Boolean);
            return normalizedHeadings.some((headingText) => titles.some((title) => headingText.includes(title)));
          }, expectedTitles[mode]);
    ...
    try {
      ...
      const otherModesUseDefault2094 = [bmPageClientSource2094, mrPageClientSource2094, gpPageClientSource2094].every(
        (source) => !source.includes('showActionButton={false}'),
      );
