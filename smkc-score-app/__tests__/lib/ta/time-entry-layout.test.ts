...
describe('TA time entry layout', () => {
  ...
  it('player-label, controls, and input constants carry expected Tailwind tokens', () => {
    expect(TA_TIME_ENTRY_PLAYER_LABEL_CLASS.split(' ')).toEqual(expect.arrayContaining(['min-w-0', 'sm:flex-1']));
    expect(TA_TIME_ENTRY_CONTROLS_CLASS.split(' ')).toEqual(expect.arrayContaining(['sm:flex', 'sm:shrink-0']));
    expect(TA_TIME_ENTRY_INPUT_CLASS.split(' ')).toEqual(expect.arrayContaining(['font-mono', 'text-sm']));
  });

  it('parses complete safe TV number integers without partial coercion', () => {
