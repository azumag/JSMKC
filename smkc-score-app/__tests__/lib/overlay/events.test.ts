...
describe('buildOverlayEvents', () => {
  ...
  it('uses an audited tie winner for the grand-final champion and scoreboard', () => {
    const events = buildOverlayEvents(
      emptyInput({
        bmMatches: [
          match({
            ...
            stage: 'finals',
            round: 'grand_final',
            player1: { id: 'upper', nickname: 'Winners' },
            player2: { id: 'lower', nickname: 'Losers' },
            score1: 0,
            score2: 0,
            winnerOverrideId: 'upper',
          }),
        ],
      }),
    );

    expect(events.find((event) => event.type === 'match_completed')?.matchResult).toEqual(
      expect.objectContaining({ winnerSide: 1 }),
    );
    expect(events.find((event) => event.type === 'mode_champion_decided')?.modeChampion?.standings?.[0]).toEqual({
      rank: 1,
      player: 'Winners',
    });
  });
  ...
  it('emits qualification_confirmed / finals_started / overall_ranking_updated when their timestamps cross since', () => {
    ...
    expect(types).toEqual(['finals_started', 'overall_ranking_updated', 'qualification_confirmed']);
  });
  ...
  it('renders TA phase-round course display with full course names', () => {
    const events = buildOverlayEvents(
      emptyInput({
        ttPhaseRounds: [{ id: 'r-kb1', phase: 'phase1', roundNumber: 1, course: 'KB1', createdAt: AFTER }],
      }),
    );
  ...
  it('emits a separate TA lives reset notification when a phase round resets lives', () => {
    ...
    expect(events.map((event) => event.type)).toEqual(['ta_phase_completed', 'ta_lives_reset']);
  ...
  it('emits a large TA champion notification with the top three standings', () => {
    ...
    expect(events.map((event) => event.type)).toEqual(['ta_phase_completed', 'ta_champion_decided']);
    expect(events[1]).toMatchObject({
