describe('TC-2078 TA E2E suite timeout contract', () => {
  it('runs the full TA suite with an explicit preview-sized timeout', async () => {
    ...
    expect(suite.tests.map((test) => test.name)).toContain('TC-2773A');
    expect(suite.tests.map((test) => test.name)).toContain('TC-2773B');
    expect(suite.tests.map((test) => test.name)).toContain('TC-2775');
    ...
    expect(suite.tests.map((test) => test.name)).toContain('TC-3047');
    expect(suite.tests).toHaveLength(41);
  });
});
