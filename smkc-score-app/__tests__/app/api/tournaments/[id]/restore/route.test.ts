...
jest.mock('@/lib/tournament-archive-restore', () => ({
  restoreTournamentArchiveForReopen: jest.fn(),
}));
...
describe('POST /api/tournaments/[id]/restore', () => {
  ...
  it('returns 404 when no archive exists', async () => {
    ...
    await POST(
      new NextRequest('http://localhost/api/tournaments/missing/restore', {
        method: 'POST',
      }),
      {
  ...
  it('reports the failed restore stage and message to the admin caller', async () => {
    jest.mocked(restoreTournamentArchiveForReopen).mockRejectedValue(
      Object.assign(new Error('too many SQL variables'), {
        restoreStage: 'BM matches',
      }),
    );
