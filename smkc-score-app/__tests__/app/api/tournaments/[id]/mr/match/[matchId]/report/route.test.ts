...
describe('MR Score Report API Route - /api/tournaments/[id]/mr/match/[matchId]/report', () => {
  ...
  describe('POST - Report match score', () => {
    it('rejects a BREAK before writing reports, logs, or standings', async () => {
      (prisma.mRMatch.findUnique as jest.Mock).mockResolvedValue({
        id: 'break-1',
        isBye: true,
        player1Id: 'p1',
        player2Id: '__BREAK__',
      ...
      const result = await POST(
        new MockNextRequest('http://localhost/report', {
          reportingPlayer: 1,
          score1: 3,
          score2: 1,
          character: 'Mario',
      ...
      expect(createScoreEntryLog).not.toHaveBeenCalled();
      expect(createCharacterUsageLog).not.toHaveBeenCalled();
      expect(prisma.mRMatch.update).not.toHaveBeenCalled();
      expect(recalculatePlayersStats).not.toHaveBeenCalled();
    });
    ...
    it('should reject participant reports after admin scoresConfirmed', async () => {
      ...
      expect(checkScoreReportAuth).toHaveBeenCalledWith(request, 't1', 1, mockMatch);
      expect(prisma.mRMatch.update).not.toHaveBeenCalled();
    });
    ...
    it.each([
      ...
      'should verify participant auth for reportingPlayer %s and return correct waitingFor value',
      async (reportingPlayer, waitingFor) => {
        const mockMatch = {
          id: 'm1',
          tournamentId: 't1',
          version: 1,
    ...
    it('should auto-confirm match when both players report matching scores', async () => {
      ...
      (prisma.mRMatch.update as jest.Mock).mockResolvedValueOnce(reportedMatch).mockResolvedValueOnce(finalMatch);
      ...
      expect(recalculatePlayersStats).toHaveBeenCalledWith(
    ...
    it('should return 404 when match does not exist', async () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/tournaments/t1/mr/match/nonexistent/report', {
        reportingPlayer: 1,
        score1: 3,
        score2: 1,
      });
