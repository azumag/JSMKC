...
describe('TA finals fetch fallback i18n', () => {
  ...
  const jaMessages = JSON.parse(
    fs.readFileSync(path.join(process.cwd(), 'messages', 'ja.json'), 'utf8'),
  ) as {
    common: { networkError?: string };
  };
  ...
  it('uses the shared localized fallback in phase 1/2 and phase 3 fetch paths', () => {
    for (const source of [eliminationPhase, finalsPage]) {
      expect(source).toMatch(
        /errorData\.error\s*\|\|\s*tCommon\('networkError'\)/,
      );
      expect(source).toMatch(
  ...
  it('removes the user-visible English fetch fallbacks without changing logger diagnostics', () => {
    expect(eliminationPhase).not.toContain(
      '`Failed to fetch ${phase} data: ${response.status}`',
    );
    expect(finalsPage).not.toContain(
      '`Failed to fetch finals data: ${response.status}`',
    );
