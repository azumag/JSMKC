...
describe('player management admin mutation error fallback contract', () => {
  ...
  const resetPasswordBlock = extractBlock(
    source,
  ...
  it('keeps duplicate nickname localization while hiding arbitrary update error detail', () => {
    expect(updateBlock).toContain('const failure = await classifyPlayerUpdateFailure(response!);');
    expect(updateBlock).toContain("failure === 'duplicateNickname'");
    ...
    expect(updateBlock).not.toContain('response!.text()');
    expect(updateBlock).not.toContain('JSON.parse');
    ...
    expect(updateBlock).toContain('kind: failure');
  });
