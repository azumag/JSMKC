...
const source = fs.readFileSync(path.join(process.cwd(), 'src/app/tournaments/[id]/gp/finals/page.tsx'), 'utf8');
...
describe('GP finals bracket creation error fallbacks (issue #3876)', () => {
  ...
  const createUpperBracket = getHandlerBlock(
    'const handleCreateUpperBracket = async () => {',
    '/** Get courses belonging to a specific cup',
  );
  ...
  it.each([
    ...
    expect(failureBranch).toContain("mode: 'gp'");
