...
describe('Prisma upstream probe diagnostic safety', () => {
  ...
  it('formats malformed JSON and transport-style failures as one bounded stderr line', () => {
    const malformedJson = new Error(
      'upstream issue response was not valid JSON: Unexpected token\n\u009b31m\u202ehidden',
    );
  ...
  it('keeps the direct single-pass entrypoint on the sanitizer for all top-level failure paths', () => {
    ...
    expect(source).toContain(
      "formatUpstreamProbeFailure('Failed to fetch Prisma upstream issue evidence', error)",
    );
