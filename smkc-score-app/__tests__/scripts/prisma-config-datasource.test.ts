...
describe('Prisma datasource config migration', () => {
  ...
  it('keeps DATABASE_URL available through prisma.config.ts without breaking generate-only CI', () => {
    expect(prismaConfigHasDatasourceUrl(prismaConfig)).toBe(true);
    expect(prismaConfig).toMatch(/url\s*:\s*process\.env\.DATABASE_URL\s*\?\?\s*["']{2}/);
  });
