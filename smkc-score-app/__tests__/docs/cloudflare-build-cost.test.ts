...
describe('Cloudflare build cost policy documentation', () => {
  ...
  it('fails closed when production preconditions cannot be verified', () => {
    expect(policy).toMatch(/If GitHub or Cloudflare data cannot be read, API\s+authorization is missing, CI state is unknown/);
  ...
  it('pins the operational reporting evidence and cost wording', () => {
    ...
    expect(policy).toMatch(/configuration state only;\s+it does not prove that the ongoing production-promotion process has operated\s+successfully/);
    expect(policy).toMatch(/distinguish actual billed or invoiced amounts from estimates\s+derived from measured build runtime/);
  });
