...
type CourseCycleMessageKey =
  'courseCycleLabel' | 'courseCycleValue' | 'availableCoursesLabel' | 'availableCoursesValue' | 'courseCycleHint';

export type CourseCycleStatusPanelTranslator = (key: CourseCycleMessageKey, values?: Record<string, number>) => string;
...
export function CourseCycleStatusPanel({ t, status, availableCoursesCount }: CourseCycleStatusPanelProps) {
    ...
    <div className="border border-foreground/15 bg-muted/30 p-3 text-sm space-y-2">
      <dl className="space-y-2">
        <div className="flex justify-between gap-3">
          <dt className="text-muted-foreground">{t('courseCycleLabel')}</dt>
        ...
        <div className="flex justify-between gap-3">
          <dt className="text-muted-foreground">{t('availableCoursesLabel')}</dt>
          <dd className="font-mono tabular-nums text-right">
      ...
      <p className="text-xs text-muted-foreground">{t('courseCycleHint', { totalPlayed: status.totalPlayed })}</p>
    </div>
  );
}
