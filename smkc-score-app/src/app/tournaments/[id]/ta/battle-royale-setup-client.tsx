...
export default function BattleRoyaleSetupClient({ tournamentId }: { tournamentId: string }) {
  ...
  useEffect(() => {
    ...
    void fetchAllPlayersForSetup<Player>()
      ...
      .catch((fetchError) => {
        logger.error('Failed to fetch players for battle royale setup', { error: fetchError, tournamentId });
  ...
  const updateHandicap = (playerId: string, taHandicapSeconds: TaHandicapSeconds) => {
    setSelectedPlayers((current) =>
      current.map((entry) => (entry.playerId === playerId ? { ...entry, taHandicapSeconds } : entry)),
    );
  };
  ...
  const startBattleRoyale = async () => {
    ...
    try {
      ...
      logger.error('Failed to start TA battle royale', { error: startError, tournamentId });
      setError(message);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="text-2xl font-semibold">{t('battleRoyaleFinals')}</h1>
            <TaModeBadge mode="battle_royale" />
          </div>
          <p className="text-sm text-muted-foreground">
            Phase 1・2と予選を行わず、選択した参加者でPhase 3を直接開始します。
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            参加者と大会内ハンデを設定してください。開始後は参加者とハンデを変更できません。
          </CardDescription>
        </CardHeader>
          ...
          {!isAdmin ? (
            ...
            <>
              ...
              <div className="flex items-center gap-3 border-b pb-3">
                ...
                />
                <Label htmlFor="battle-royale-select-all" className="cursor-pointer">
                  {t('selectAll')}
                </Label>
                <span className="ml-auto text-sm text-muted-foreground">{selectedPlayers.length}人選択</span>
              </div>
                    ...
                    return (
                      <div key={player.id} className="flex min-h-14 items-center gap-3 px-3 py-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => togglePlayer(player, checked === true)}
                        />
                        <Label htmlFor={`battle-royale-player-${player.id}`} className="min-w-0 flex-1 cursor-pointer">
                          <span className="block truncate font-medium">{player.nickname}</span>
                          <span className="block truncate text-xs text-muted-foreground">{player.name}</span>
                        </Label>
                        {selected && (
                          <TaHandicapSelect
                            value={selected.taHandicapSeconds}
                            onValueChange={(value) => updateHandicap(player.id, value)}
              ...
              <div className="flex justify-end">
                <Button onClick={() => setConfirmOpen(true)} disabled={selectedPlayers.length < 2 || saving}>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('startBattleRoyaleConfirmTitle')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('startBattleRoyaleConfirmDesc', { count: selectedPlayers.length })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={saving}>{tc('cancel')}</AlertDialogCancel>
            <AlertDialogAction disabled={saving} onClick={(event) => { event.preventDefault(); void startBattleRoyale(); }}>
