...
export default function TournamentLayout({
  ...
  const fetchTournament = useCallback(async () => {
    try {
      const response = await fetchWithRetry(
        `/api/tournaments/${id}?fields=summary`,
        { cache: "no-store" },
      );
      if (response.ok) {
  ...
  const updateStatus = async (status: string) => {
    ...
    try {
      ...
      const updatedTournament =
        await parseTournamentStatusUpdateResponse<Tournament>(response);
  ...
  const { guardClassName, ...tabHydrationGuardProps } =
    getTabHydrationGuardProps(tabsHydrated);
  const canManageStatus =
    Boolean(isAdmin) && canUpdateTournamentStatus(tournament);
  const visibleModeTabs = tournament.taBattleRoyaleMode
    ? TABS.filter((tab) => tab.href === "ta")
    : TABS;
        ...
        <header className="border-b border-foreground/15 pb-5">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            <div className="flex items-stretch gap-3">
              <span
                aria-hidden="true"
                className="block w-[3px] bg-primary self-stretch"
              />
              <div>
            ...
            <div className="flex flex-col items-start lg:items-end gap-2">
              ...
              {statusError ? (
                <p
                  role="alert"
                  className="max-w-xl text-sm text-red-600 lg:text-right"
                >
                  {statusError}
                </p>
              ) : null}
