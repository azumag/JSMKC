...
export default function OverallRankingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const {
    ...
    error: pollError,
    refetch,
  } = usePolling(fetchRankings, { interval: POLLING_INTERVAL, cacheKey: `tournament/${tournamentId}/overall-ranking` });
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 border-b border-foreground/15 pb-4">
        <div>
          <h1 className="font-display text-3xl sm:text-4xl tracking-wide leading-none">{tOverall('title')}</h1>
          <p className="text-sm text-muted-foreground mt-2">{tOverall('subtitle')}</p>
          {lastUpdated && (
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {isAdmin && <ModePublishSwitch tournamentId={tournamentId} mode="overall" modeLabelKey="overall" />}
        </div>
      </header>
        ...
        <>
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-auto">
                    ...
                    <p className="text-xs opacity-80 mt-0.5">{ranking.playerName}</p>
                    <p className="font-mono tabular text-2xl mt-2">{ranking.totalPoints.toLocaleString()}</p>
                    <p className="text-xs opacity-70 mt-0.5">{tOverall('totalPoints')}</p>
                  </div>
                </article>
              );
            })}
          ...
          <section className="space-y-3">
            <header className="flex items-baseline justify-between">
              <h2 className="text-base font-semibold">{tOverall('completeRankings')}</h2>
          ...
          <section className="border border-foreground/15 p-6">
            <h2 className="text-base font-semibold mb-4">{tOverall('pointsSystem')}</h2>
