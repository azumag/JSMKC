...
export default function MatchRacePageClient({
  ...
  tournamentId: string;
  initialData?: QualInitialData | null;
}) {
   ...
   */
  const qualifications: MRQualification[] = pollData?.qualifications ?? EMPTY_MR_QUALIFICATIONS;
  ...
  const handleBroadcastMatch = async (match: MRMatch, score1: number, score2: number) =>
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" asChild>
            <a href={`/tournaments/${tournamentId}/mr/participant`}>{tc('enterScore')}</a>
          </Button>
          ...
          {finalsExists === true ? (
            <Button variant="outline" asChild>
              <a href={`/tournaments/${tournamentId}/mr/finals`}>{tc('viewTournament')}</a>
            </Button>
          ) : canCreateFinals ? (
            <Button
          ...
          {isAdmin && <ModePublishSwitch tournamentId={tournamentId} mode="mr" modeLabelKey="matchRace" />}
        </div>
      </div>
                              ...
                              <TableBody>
                                {matchesByDay[day].map((match) => (
                                  <TableRow key={match.id} className={match.isBye ? 'opacity-50 bg-muted/30' : ''}>
                                    ...
                                    <TableCell className={match.completed && match.score1 >= 3 ? 'font-bold' : ''}>
                                      {match.player1.nickname}
                                    </TableCell>
                                    <TableCell className="text-center font-mono">
                                      {match.isBye || match.completed ? `${match.score1} - ${match.score2}` : '- - -'}
                                    </TableCell>
                                    <TableCell
                                      className={
                                        !match.isBye && match.completed && match.score2 >= 3 ? 'font-bold' : ''
                                      }
                                    >
          ...
          <div className="space-y-4 py-4">
            ...
            {selectedMatch &&
              (() => {
                ...
                if (!prevMatch) {
                  return (
                    <p className="text-sm text-muted-foreground text-center mb-2">{tc('characterPriorityFirst')}</p>
                  );
                }
                ...
                const loserNickname =
                  p1Score < p2Score ? selectedMatch.player1.nickname : selectedMatch.player2.nickname;
                return (
                  <p className="text-sm text-blue-600 text-center mb-2">
                    {tc('characterPriority', { player: loserNickname })}
                  </p>
                );
              })()}
