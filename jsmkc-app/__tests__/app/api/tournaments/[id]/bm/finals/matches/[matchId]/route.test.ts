/**
 * @module BM Finals Match Update API Route Tests
 *
 * Test suite for the individual Battle Mode finals match update endpoint:
 * /api/tournaments/[id]/bm/finals/matches/[matchId]
 *
 * This file covers the PUT method which updates score and completion status for a specific
 * finals match. Admin authentication is required. The endpoint supports auto-completion
 * when a player reaches the target win count (5 wins in finals), manual completion override,
 * and round-by-round data tracking.
 *
 * Key behaviors tested:
 *   - Successful match score update with valid data and admin authentication
 *   - Auto-completion when player1 or player2 reaches target wins (5)
 *   - No auto-completion when scores are below target
 *   - Manual completion via explicit completed flag
 *   - Rounds data update (arena and winner per round)
 *   - Authentication enforcement: 401 for unauthenticated, missing user, and non-admin roles
 *   - Validation: score1/score2 out of range (0-5), invalid rounds data (invalid arena type,
 *     invalid winner value)
 *   - 404 handling when match does not exist
 *   - Database error handling with structured logging
 *   - Invalid JSON request body handling
 *   - IP address resolution via x-forwarded-for header
  ...
  describe('PUT - Update finals match', () => {
    ...
    it('should update match score with valid data and admin authentication', async () => {
      ...
      /* Source resolves IP from x-forwarded-for || x-real-ip || "unknown".
         This request only has user-agent, so falls back to "unknown". */
