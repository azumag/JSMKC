...
jest.mock('@/lib/prisma', () => ({
  default: {
    ...
    bMMatch: { findMany: jest.fn(), findUnique: jest.fn(), findFirst: jest.fn(), update: jest.fn(), create: jest.fn(), deleteMany: jest.fn() },
...
describe('BM Finals API Route - /api/tournaments/[id]/bm/finals', () => {
  ...
  describe('POST - Create finals tournament from qualification results', () => {
    ...
    it('should use default topN of 8 when not provided in body', async () => {
      const mockQualifications = [
        { id: 'q1', tournamentId: 't1', playerId: 'p1', score: 10, points: 20, player: { id: 'p1' } },
      ];
  ...
  describe('PUT - Update match result and advance players', () => {
    ...
    it('should enable reset match when losers bracket winner wins grand final', async () => {
      ...
      const mockResetMatch = { id: 'm51', player1Id: null, player2Id: null };
      
      (prisma.bMMatch.findUnique as jest.Mock).mockResolvedValue(mockMatch);
      (prisma.bMMatch.update as jest.Mock).mockResolvedValue({ ...mockMatch, completed: true });
      (prisma.bMMatch.findFirst as jest.Mock).mockResolvedValue(mockResetMatch);
      
      const request = new MockNextRequest('http://localhost:3000/api/tournaments/t1/bm/finals', { matchId: 'm50', score1: 1, score2: 3 });
      const params = Promise.resolve({ id: 't1' });
      const result = await PUT(request, { params });
    ...
    it('should detect tournament completion when winners bracket player wins grand final', async () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/tournaments/t1/bm/finals', { matchId: 'm50', score1: 3, score2: 1 });
      const params = Promise.resolve({ id: 't1' });
      const result = await PUT(request, { params });
    ...
    it('should detect tournament completion after reset match is played', async () => {
      ...
      const request = new MockNextRequest('http://localhost:3000/api/tournaments/t1/bm/finals', { matchId: 'm51', score1: 3, score2: 2 });
      const params = Promise.resolve({ id: 't1' });
      const result = await PUT(request, { params });
