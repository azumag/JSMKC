...
jest.mock('@/lib/prisma', () => ({
  default: {
    gPQualification: { findMany: jest.fn(), deleteMany: jest.fn(), updateMany: jest.fn(), create: jest.fn() },
    gPMatch: { findMany: jest.fn(), findUnique: jest.fn(), update: jest.fn(), deleteMany: jest.fn(), create: jest.fn() },
...
describe('GP API Route - /api/tournaments/[id]/gp', () => {
  ...
  describe('PUT - Update match score with race data', () => {
    ...
    it('should calculate correct driver points for each race', async () => {
      ...
      expect(updateCall[0].data.points1).toBe(27);
