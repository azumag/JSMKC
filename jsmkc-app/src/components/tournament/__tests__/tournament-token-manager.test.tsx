...
expect(navigator.clipboard.writeText).toHaveBeenCalledWith(mockToken);
  ...
  describe("Unauthorized State", () => {
    it("should render unauthorized message when user is not logged in", () => {
      require("next-auth/react").useSession.mockReturnValue({
        data: null,
      });
    ...
    it("should not show token management UI when unauthorized", () => {
      require("next-auth/react").useSession.mockReturnValue({
        data: null,
      });
      ...
      expect(screen.queryByText("Tournament Token Management")).not.toBeInTheDocument();
    });
  });

  describe("Authorized State - No Token", () => {
    beforeEach(() => {
      require("next-auth/react").useSession.mockReturnValue({
        data: {
          user: { id: "1", name: "Test User", email: "test@example.com" },
    ...
    it("should render token management UI for authorized users", () => {
      render(<TournamentTokenManager tournamentId={mockTournamentId} />);
    ...
    it("should show inactive status when no token exists", () => {
      render(<TournamentTokenManager tournamentId={mockTournamentId} />);

      expect(screen.getByText("Inactive/Expired")).toBeInTheDocument();
    });

    it("should show 'No expiry set' when no token exists", () => {
      render(<TournamentTokenManager tournamentId={mockTournamentId} />);

      expect(screen.getByText("No expiry set")).toBeInTheDocument();
    });

    it("should not show token display section when no token exists", () => {
      render(<TournamentTokenManager tournamentId={mockTournamentId} />);

      expect(screen.queryByText("Token:")).not.toBeInTheDocument();
    });

    it("should not show participant URL section when no token exists", () => {
      render(<TournamentTokenManager tournamentId={mockTournamentId} />);

      expect(screen.queryByText("Participant Score Entry URL:")).not.toBeInTheDocument();
    });

    it("should render regenerate and extend buttons", () => {
      render(<TournamentTokenManager tournamentId={mockTournamentId} />);

      expect(screen.getByText("Regenerate Token")).toBeInTheDocument();
      expect(screen.getByText("Extend by 24h")).toBeInTheDocument();
    });
  });

  describe("Authorized State - Active Token", () => {
    beforeEach(() => {
      require("next-auth/react").useSession.mockReturnValue({
        data: {
          user: { id: "1", name: "Test User", email: "test@example.com" },
    ...
    it("should show active status when valid token exists", () => {
      render(
        <TournamentTokenManager 
          tournamentId={mockTournamentId}
          initialToken={mockToken}
...
it("should show token display section", () => {
      render(
        <TournamentTokenManager 
          tournamentId={mockTournamentId}
          initialToken={mockToken}
    ...
    it("should show participant URL section", () => {
      render(
        <TournamentTokenManager 
          tournamentId={mockTournamentId}
          initialToken={mockToken}
      ...
      expect(screen.getByText("Participant Score Entry URL:")).toBeInTheDocument();
    });
  ...
  describe("Token Display - Show/Hide Toggle", () => {
    beforeEach(() => {
      require("next-auth/react").useSession.mockReturnValue({
        data: {
          user: { id: "1", name: "Test User", email: "test@example.com" },
...
describe("Copy to Clipboard", () => {
    ...
    it("should copy token to clipboard when copy button is clicked", () => {
      ...
      jest.spyOn(require("sonner").toast, "success").mockImplementation(toastSuccessSpy);
    ...
    it("should not show error toast when URL clipboard copy fails", () => {
      ...
      (require("sonner").toast.error as jest.Mock) = toastErrorSpy;
      
      // Mock clipboard to reject for URL copy
...
it("should show 'Expired' when token is past expiry", () => {
      ...
      const timeRemainingElement = screen.getByText(/Time Remaining/).parentElement;
      expect(timeRemainingElement).toContainElement(screen.getByText(/Expired/));
    });
    ...
    it("should show days and hours when more than 24 hours remaining", () => {
      ...
      expect(screen.getByText(/2 days 0h remaining/)).toBeInTheDocument();
    });
