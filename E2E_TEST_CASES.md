...
### TC-2585: TC-2577/TC-2578 モデルルーティング検証を強化する（bound 関数経由の正の assertion）
- **背景**: issue #2585。TC-2577/TC-2578 の初期実装では `expect.objectContaining({ findMany, count })` という弱い assertion を使用しており、どの Prisma モデルでも一致してしまうため、ルーティングの正確性を保証できていなかった。TC-2578 は負の assertion のみで、gPMatch が実際に `paginate` へ渡されたことを確認していなかった。`.bind()` で作成された wrapper 関数を実際に呼び出すことで、元の jest モック (`prisma.mRMatch.findMany` / `prisma.gPMatch.findMany`) が委譲先として使われているかを正確に検証する。
- **手順**: 既存 TC-2577/TC-2578 の assertion を強化する。`paginate` の第 1 引数から `findMany` ラッパーを取得し、それを呼び出して `prisma.mRMatch.findMany` (TC-2577) / `prisma.gPMatch.findMany` (TC-2578) が実際に呼ばれたことを確認する。
- **期待結果**: TC-2577 は `prisma.mRMatch.findMany` が呼ばれ、`bMMatch.findMany` / `gPMatch.findMany` は呼ばれない。TC-2578 は `prisma.gPMatch.findMany` が呼ばれ、`bMMatch.findMany` / `mRMatch.findMany` は呼ばれない。
- **スクリプト**: n/a (unit/static coverage) — smkc-score-app/__tests__/lib/api-factories/matches-polling-route.test.ts
