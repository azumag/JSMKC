...
- **スクリプト**: n/a（ライブE2E未実施 — 本セッションではPlaywright実行環境を使わずコード変更のみ）。ユニット: smkc-score-app/__tests__/lib/ranking-utils.test.ts（`computeCombinedRanks`、doc §2.3のワークサンプルと同じ数値を使用）, smkc-score-app/__tests__/lib/finals-group-selection.test.ts（`selectFinalsEntrantsByGroup` 3/4グループ分岐のバケットタイブレーク）
