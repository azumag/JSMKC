...
## TC-523: BM 決勝スコアダイアログ — TV# 保存ボタンがスコア送信せずに永続化する (issue #651)
- **背景**: スコアダイアログに「TV# 保存」ボタンがあり、スコアを確定せずに TV 番号だけを保存できる
- **手順**:
  1. 8名ブラケットを生成し `/bm/finals` に移動
  2. `bracket-match-card` をクリックしてスコアダイアログを開く
  3. TV# ドロップダウンで TV#3 を選択し「TV# 保存」ボタンをクリック
  4. ダイアログが閉じないことを確認（スコア送信は行われていない）
  5. API で matchNumber=1 の tvNumber が 3 に更新されていることを確認
- **期待結果**: ダイアログを開いたまま TV 番号だけが永続化される
...
- **期待結果**: イベントが現れる
- **備考**: PUT 経路の `recalculateRanks` が times から totalTime を再計算するため、部分的な time map（例: 4 コースだけ）だと totalTime=null になりイベントが出ない
...
- **背景**: TC-910 が TA エントリーに全 20 コースのタイムを投入すると `ta_time_recorded` イベントが発生する。このイベントはタイムラインで `dashboard-timeline-ta-time` リッチカードとして描画される（コース名・タイム・順位バッジ付き）
