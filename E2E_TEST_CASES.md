...
## TC-2126: TC-939 reporting contract — lib test に一元化する
- **URL**: n/a (unit/static coverage)
- **authRequired**: false
- **背景**: issue #2126。`__tests__/e2e/tc-939-reporting.test.ts` は `__tests__/lib/tc939-reporting.test.ts` の「両方の失敗理由を報告する」ケースと重複しており、TC-939 reporting helper の contract は lib 単体テストに集約した方が保守しやすい。
- **手順**:
  1. `__tests__/e2e/tc-939-reporting.test.ts` が存在しないことを確認する
  2. `__tests__/lib/tc939-reporting.test.ts` が reload と className failure の同時報告を検証していることを確認する
  3. TC-939 の E2E シナリオが削除済み E2E test ではなく `tc-all.js` と lib/static guard を参照していることを確認する
- **期待結果**:
  - TC-939 reporting helper の failure detail contract は lib test に一元化される
  - 削除済み E2E test への参照が docs や guard に戻った場合は検知される
  - TC-939 本体の E2E 登録と tab Link static guard は従来通り維持される
- **スクリプト**: `npm test -- --runTestsByPath __tests__/static/tc-2126-tc939-reporting-consolidation.test.ts __tests__/lib/tc939-reporting.test.ts __tests__/docs/e2e-cases-drift.test.ts`
...
- **スクリプト**: tc-all.js TC-939 / `__tests__/lib/tc939-reporting.test.ts` / `__tests__/static/tc-939-tournament-tabs-link.test.ts`
