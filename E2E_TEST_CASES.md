...
- **手順**:
  ...
  4. Amazon 外部リンクの `href` と `target="_blank"` を確認する
  5. 外部リンクに `rel="sponsored noopener noreferrer"` が指定されていることを確認する
- **期待結果**: おすすめセクションが日英の翻訳キーを通じて表示され、Amazon 外部リンクは sponsored disclosure と安全な `rel` 属性を維持する
- **スクリプト**: `__tests__/e2e/tc-1417-home-recommendation.test.ts`
