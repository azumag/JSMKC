...
- **期待結果**: 誤って早期昇格した場合でも、対象フェーズのエントリとラウンド結果を「フェーズリセット」で削除し、前フェーズの結果確定後に正しい人数で再昇格できる。後続フェーズ（フェーズ3など）が既に存在する場合はリセットが409で拒否される（`src/lib/ta/finals-phase-manager.ts` の `resetPhase` / `PhaseResetConflictError`）。
