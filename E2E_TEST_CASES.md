...
## TT (Time Trial / TA) フルワークフローテスト  *(TC-801/802/804/805/806/807/808 実装済み)*
...
## TC-802: TAプレイヤーログインからタイム入力 ✅ 実装済み
- **URL**: /auth/signin -> /tournaments/[temp-id]/ta/participant
- **authRequired**: true (player)
- **背景**: プレイヤーcredentialsログイン後、TA participant UI から20コース分のタイムを一括送信し、APIに反映されることを確認する
- **手順**:
...
### TC-802 実装済み
- TC-802 TAプレイヤーログインからタイム入力: `tc-ta.js` に追加済み（playerログイン + participant UI 20コース送信 + API反映確認）
