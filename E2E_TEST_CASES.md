...
### TC-1528: TA 決勝フェーズ — API ラウンド送信ヘルパーの自動/明示コース経路を共通化する
- **URL**: `/api/tournaments/[id]/ta/phases`
- **背景**: issue #1528。`tc-ta.js` の自動コース選択経路とコース明示経路はどちらも `start_round` 後に同じ `roundNumber` で `submit_results` するため、片方だけ変更されると TA 決勝 E2E のセットアップ挙動が分岐しやすい。
- **手順**:
...
- **期待結果**: 2つの公開 helper は同じ内部 start/submit 実装を使い、API payload と戻り値互換性が維持される
