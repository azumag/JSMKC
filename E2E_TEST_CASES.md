...
CDM（大会ライブ運営）中に予期せぬ事態（シード表の取り違え・棄権/失格による交代・誤配置の訂正）が発生した際、記録済みスコアを失わずに未完了マッチのスロット(`player1Id`/`player2Id`)を管理者が手動修正できる機能。既存の決勝PATCH(`/api/tournaments/[id]/{bm|mr|gp}/finals`)に`slotEdit`(`assign`/`swap`/`swapSlots`)を追加し、`applySlotWrite`統一による楽観的ロック(`version`)で自動進出との競合を検知する。詳細設計はissue #3017参照。Phase 1はBMブラケットUIのみ配線(MR/GPはAPI共通で有効だがUI配線はPhase 2)。
