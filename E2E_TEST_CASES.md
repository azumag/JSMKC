...
- **手順**:
  ...
  4. playoff 完了後に Phase 2 finals を生成し、direct seed 側（1〜12）も同様に数値シードが優先表示されることを確認する
...
- **URL**: /api/tournaments/[id]/bm/finals (POST)
- **authRequired**: true (admin)
- **背景**: issue #1053。Top-24 Phase 2 は `generatePlayoffStructure(12)` の `playoff_r2` 4試合にある `advancesToUpperSeed` を使い、playoff 勝者（バイシード自身の実シード番号）を Upper Bracket の seed 16/13/15/14 に配置する。構造定義の変更やバグで `playoffUpperSeeds` が4件そろわない場合、勝者を欠落させたままブラケットを作ると原因調査が難しい。
- **手順**:
...
- **URL**: /api/tournaments/[id]/bm/finals (POST)
- **authRequired**: true (admin)
- **背景**: issue #1051。Top-24 の direct advancer は Upper Bracket seed 1〜12（連番）に、barrage entrant は seed 13〜24（連番）に配置される（CDM 2025公式結果で検証済み。旧・グループ対戦回避のための飛び番配置は廃止）。呼び出し側に必要なのは seed 付きの `directSeeds`/`barrageSeeds` のみであり、冗長な `direct[]`/`barrage[]` 配列は公開しない。
- **手順**:
...
- **背景**: `docs/qualification-combined-ranking.md` の調査で、CDM Excelテンプレート（大会運営が実際に使う`cdm-2025-template.xlsm`）の2グループ合算順位数式には同バケット内のタイブレークが一切なく、常にグループ文字順（A→B）で確定してしまう不具合が判明した（例: グループ内4位で勝点が上回るB4がA4より下に来てしまう）。大会運営に確認の上、シードランキングは使わず「WDLスコア→得失点差」のタイブレークをグループ数によらず統一採用する決定がされた（同doc §7 Q1/Q2）。本TCはアプリ側の合算順位表（BM/MR/GP予選ページの「合算順位」タブ、`computeCombinedRanks()` が算出）がこの決定通りに動作することを検証する。3グループ以上の決勝進出選定（`selectFinalsEntrantsByGroup()`）についても同様の修正が入っている（`docs/qualification-combined-ranking.md` §2-§3）。**更新（TC-3010）**: 当初はUIからの3グループ作成が `LOCKED_GROUP_COUNT=2` でブロックされていたためユニットテストのみのカバーだったが、TC-3010でグループ数セレクタが2/3に解禁されたため、3グループの決勝進出選定は今後ライブE2Eでもカバー対象になる。
...
- **自動化TODO**: `e2e/tc-bm.js` に3グループ24名以上のfixtureを追加し、A/B/C作成、合算順位、バラージ12名（seed13〜24）、直接進出12人（seed1〜12）まで検証する。
  - **注記（CDM 2025公式結果による是正）**: 当初この項目は「Upper Bracket 1回戦で同グループ対決が発生しないこと」も検証予定としていたが、CDM 2025公式結果ワークブックの検証で、実際の大会では同グループ対決が普通に発生していたことが判明した（例: Drew vs Zarkov、Lafungo vs Moll、いずれもグループ内対戦）。同グループ対戦回避の仕組み（`assignAntiCollisionSeeds()`等）は運営確認済みの決定ではなく検証されていない暫定実装だったため撤去し、単純な連番バケット方式（seed 1〜12直接・13〜24バラージ）に統一した。したがって同グループ対戦は起こり得る仕様として扱い、回避の検証は行わない。
