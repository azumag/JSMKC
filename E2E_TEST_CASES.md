...
- **手順**:
  1. `e2e-cases-drift.test.ts` が `overlayFinalsSelects.length` を 3 件以上として検証することを確認する
  2. 各 select が `stage` / `round` / `createdAt` を含むことを確認する
  3. TC-2224 の drift guard が TC-2196 の select 件数アサーションを直接参照することを確認する
...
### TC-2265: server-ranking の rankOverride 判定フラグ名を値と区別する
- **背景**: issue #2265。`server-ranking.ts` の `rankOverride` 判定 boolean が `overrideRank` という名前だと、実際の順位値である `rankOverride` と読み違えやすい。
- **手順**:
  1. `assignRanksForPartition` の override 判定が `hasRankOverride` という boolean 名で表現されていることを確認する
  2. そのフラグが `rankOverride != null` だけを表し、`if (hasRankOverride)` で分岐していることを確認する
  3. `const overrideRank = entry.rankOverride != null;` が再導入されていないことを静的ガードで確認する
- **期待結果**: rank override の有無を示す boolean と実際の override rank 値を読み違えない
- **スクリプト**: n/a (static coverage) — smkc-score-app/__tests__/static/tc-2265-server-ranking-rank-override-name.test.ts
