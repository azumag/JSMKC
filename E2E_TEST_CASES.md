...
- **URL**: `/tournaments/[id]/ta/phase1`, `/api/tournaments/[id]/ta/phases`
- **背景**: issue #1033。`TieBreakDecision.reason` は呼び出し元・DB・APIで未使用の内部診断候補で、公開payloadに出す必要がない。YAGNIに従い、サドンデス開始時に必要な `targetPlayerIds` だけを扱う。
- **手順**:
  1. 24名 TA 予選を作成し、Phase1 に17-24位の8名を昇格する
  2. Phase1 ラウンド1で最下位2名を同タイムにしてサドンデスを発生させる
