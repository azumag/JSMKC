...
- **手順**:
  ...
  7. 補助テストでは TC-939 reporting helper を describe スコープの共通初期化から呼び出し、各 failure matrix が同じ helper 経由で検証されることを確認する
- **期待結果**: タブクリックは full document reload ではなく SPA navigation として完了し、hydrated 後のタブ className に guard 由来の余分な空白が残らない。両方の失敗条件が同時に発生した場合は、reload と className の両方の理由を同じ TC-939 ログに出力する。補助テストは import 宣言を各 failure case に重複させず、共通初期化経由で同じ契約を検証する
