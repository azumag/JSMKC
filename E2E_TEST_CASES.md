...
### TC-2493: resolveAuditUserId — null セッションは undefined を返す
...
- **手順**: `resolveAuditUserId(null)` および `resolveAuditUserId(undefined)` を呼び出す。
- **期待結果**: 両ケースで `undefined` が返る。
- **スクリプト**: n/a (unit/static coverage) — smkc-score-app/__tests__/lib/audit-log.test.ts

---

### TC-2494: resolveAuditUserId — user が null のセッションは undefined を返す
- **背景**: `session.user` が null または undefined の場合、FK 違反を防ぐため undefined を返す必要がある。
- **手順**: `resolveAuditUserId({ user: null })` および `resolveAuditUserId({ })` を呼び出す。
- **期待結果**: 両ケースで `undefined` が返る。
- **スクリプト**: n/a (unit/static coverage) — smkc-score-app/__tests__/lib/audit-log.test.ts
...
- **手順**: `resolveAuditUserId({ user: { id: 'admin-user-id', userType: 'admin' } })` を呼び出す。
- **期待結果**: `'admin-user-id'` が返る。
- **スクリプト**: n/a (unit/static coverage) — smkc-score-app/__tests__/lib/audit-log.test.ts
