...
- **手順**:
  ...
  2. コース `MC1` を手動指定し、**同着なしの相異なるタイム**で通常ラウンドを送信する
  3. サドンデス対象が p3/p4 の2名で、`course` がベースコース `MC1` 以外の新規コースであることを確認する
  4. 銅メダルサドンデスで p4 が p3 より速いタイムを送信して解決する
...
- **期待結果**: 同時最終ライフ喪失は即敗退させず銅メダルサドンデスへ移行し、その結果で3位/4位を確定する。銅メダルコースはプールから消費され、生存2名はトップ2へ進出する
