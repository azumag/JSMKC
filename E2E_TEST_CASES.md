...
- **手順**: `recommendGroupCount(15)` を呼び出す。
- **期待結果**: `2` が返る。
- **スクリプト**: n/a (unit/static coverage) — smkc-score-app/__tests__/components/tournament/group-setup-dialog.test.ts
...
### TC-2923: recommendGroupCount — 0名でも最小2グループを返す
- **背景**: 参加者なしの場合でも有効な最小グループ数を返すことを保証する。
- **手順**: `recommendGroupCount(0)` を呼び出す。
- **期待結果**: `2` が返る。
- **スクリプト**: n/a (unit/static coverage) — smkc-score-app/__tests__/components/tournament/group-setup-dialog.test.ts
