...
- **背景**: issue #825。Prisma 側の `0010_ta_phase_sudden_death` migration が `JSONB` を使うと、SQLite/D1 用の Wrangler migration (`0028_ta_phase_sudden_death.sql`) と型定義がズレる。
- **手順**:
  ...
  4. 対象 migration に `JSONB` が残っていないことを確認する
- **期待結果**: TA sudden-death と GP finals の JSON 格納列は SQLite/D1 互換の `TEXT` として migration に記録される
