...
- **手順**:
  ...
  3. 無認証 GET でポーリングし、`type='match_completed'`, `mode='bm'`, `subtitle` に "4-0", `title` に "BM" または "Battle Mode" と、完了を示す文言が含まれるイベントが現れること
...
- **手順**: MR 予選試合に admin PUT (3-1) → unauth poll で `type='match_completed'`, `mode='mr'`, subtitle に `3-1`, title に `MR` または `Match Race` と完了を示す文言を含むイベントが現れる
- **期待結果**: 15 秒以内に取得できる
...
- **期待結果**: subtitle に `45-0`（5×9pts）形式のスコア、title に `GP` または `Grand Prix` と完了を示す文言を含む
