...
- **手順**:
  ...
  7. legacy/divergent な `assignedCups` バックフィルが必要な場合、GET はラウンド内の修復対象 update を逐次待ちせず並列に開始することを単体テストで確認する
  8. 一部 update が失敗してもGETレスポンスは返し、失敗件数と理由を警告ログに残すことを単体テストで確認する
  9. 管理者スコア入力ダイアログを開き、FT2 は2カップ欄、FT3 は3カップ欄が最初から表示され、その自動表示分に削除ボタンがないことを確認
  10. クリーンアップ
- **期待結果**: ラウンドをまたいだ同カップは許可されるが、同じラウンドのカップ組み合わせは FT3 の5件目を除いて重複しない。FT数ぶんの初期カップ欄は削除できず、追加したカップ欄だけ削除できる。legacy 修復時のDB updateは全件を開始してから完了待ちし、一部失敗しても閲覧レスポンスを継続する
