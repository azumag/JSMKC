...
- **手順**:
  ...
  9. `seededPlayers` の direct seed 1〜12 が紙配置に対応する seed 順、seed 13〜16 が `playoff_r2` 勝者4名で埋まることを確認
  10. クリーンアップ
...
### TC-510 実装済み
- TC-510 BM Top-24 バラージ（Pre-Bracket Playoff）: `tc-bm.js` に `topN: 24` の playoff 作成、2グループ手書き紙配置、未完了409、R1→R2ルーティング、R2完了、Top-16 finals 生成、seed 13〜16 割当チェックを追加
...
### TC-816: TA/TT 決勝フェーズ — サドンデスコースは通常ラウンド20本サイクルを消費しない
- **URL**: `/tournaments/[id]/ta/finals`, `/api/tournaments/[id]/ta/phases`
- **背景**: 通常ラウンドはフェーズ1-3通算の20コースを重複なしで消化する。サドンデスで使ったコースはタイブレーク用であり、通常ラウンドの20本サイクルを進めてはいけない。
- **手順**:
  1. Phase1/Phase2/Phase3 通算で通常ラウンド17本を作成し、サドンデス3本を解決済みにする
  2. `GET /api/tournaments/[id]/ta/phases?phase=phase3` を実行する
  3. `playedCourses` が通常ラウンド17本のみで、`availableCourses` に残り3コースが含まれることを確認する
  4. 次の通常ラウンド開始時に、サドンデスで使っただけのコースが候補から除外されないことを確認する
- **期待結果**: サドンデス3本を足して20本扱いにせず、通常ラウンドで未使用のコースが20本目まで選択可能なまま残る
- **スクリプト**: scenario only
