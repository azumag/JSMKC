...
- **背景**: SSR（TC-905）はシェルが返るだけ。クライアントのポーリング → toast 描画パイプライン全体が動くことを保証する
- **手順**:
  1. Playwright で `/tournaments/[id]/overlay` を開く
  2. `[data-testid="overlay-root"]` が hydrate するのを待つ
  3. 管理 API で BM 予選試合のスコアを 3-1 に更新（4-0 から差し替え、再 PUT で `updatedAt` を更新）
  4. 15 秒以内に `[data-testid="overlay-toast"]` が DOM に出現
  5. スタック全体のテキストに `BM` / 数字-数字スコア / `終了` が含まれる
- **期待結果**: 実ブラウザでトーストが表示される
- **備考**: BM 予選は `score1 + score2 === 4` 制約があるため、再 PUT 値は 3-1 / 2-2 / 4-0 など 4 に総和するもののみ有効
