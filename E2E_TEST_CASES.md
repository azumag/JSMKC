...
- **手順**:
  ...
  4. 各ページで「Failed to fetch」「500」エラーが表示されないこと
...
## TC-356: TA決勝フェーズ — プレイヤーごとのTV選択と「配信に反映」ボタン (issue #800)
- **authRequired**: true (admin)
- **背景**: TA決勝（Phase1/2/3）のラウンド開始後、各プレイヤーにTVナンバー（1-4）を割り当て「配信に反映」ボタンで broadcast API に反映できる。TVナンバーはラウンド開始前ではなくラウンド中に入力する。
- **手順**:
  1. TA決勝フェーズのラウンドを開始（ラウンド画面に遷移）
  2. タイム入力フォームに各プレイヤーのTVセレクタ（TV1〜TV4）が表示されること
  3. プレイヤー1 → TV1, プレイヤー2 → TV2 を選択
  4. 「配信に反映」ボタンをクリック
  5. `GET /api/tournaments/[id]/broadcast` で `player1Name` = プレイヤー1のニックネーム、`player2Name` = プレイヤー2のニックネームが返ること
  6. ラウンド結果提出後、次ラウンド開始前にTVセレクタがリセットされること
- **期待結果**:
  - ラウンド画面にプレイヤーごとのTVセレクタが表示される
  - 「配信に反映」ボタンが broadcast API に TV1→1P, TV2→2P として反映する
  - ラウンド結果提出後にTVセレクタは初期化される（次ラウンドは未選択）
  - ラウンド開始前のプリラウンド画面にTVセレクタは表示されない
- **スクリプト**: tc-all.js TC-356 (E2E手動 → 自動化予定)
...
### TC-816: TA/TT 決勝フェーズ — フェーズ間コース履歴を引き継ぐ
- **URL**: `/tournaments/[id]/ta/phase1`, `/tournaments/[id]/ta/phase2`, `/api/tournaments/[id]/ta/phases`
- **背景**: Phase1 の通常ラウンドとサドンデスで使用したコースは、Phase2 の利用可能コースからも除外される必要がある。
- **手順**:
  1. Phase1 に8名を用意し、通常ラウンドを開始する
  2. 最下位同着でサドンデスを発生させ、サドンデスのコースを変更して解決する
  3. Phase1 を4名まで完了する。通常ラウンド送信がHTTPエラーか予期しない同着かを別々のエラーとして扱う
  4. Phase2 へ昇格する
  5. Phase2 の playedCourses / availableCourses を確認する
- **期待結果**: Phase1 通常ラウンドとサドンデスで使ったコースが Phase2 の playedCourses に含まれ、availableCourses から除外される
- **スクリプト**: tc-ta.js TC-816

### TC-926: TA/TT E2E — TypeScript helper の CommonJS 実行時解決
- **URL**: なし（E2E runner 起動時の実行時 smoke）
- **背景**: `tc-ta.js` は CommonJS から拡張子なしで `e2e/lib/*.ts` helper を読み込むため、`ts-node/register/transpile-only` の登録順が崩れると `node --check` では検出できない実行時エラーになる。
- **手順**:
  1. `tc-ta.js` 起動後に拡張子なし require で `layout-assertions` と `ta-phase-assertions` を解決する
  2. それぞれの exported assertion が関数として参照できることを確認する
- **期待結果**: TypeScript E2E helper が CommonJS 実行時に解決され、TA suite 起動時に module-not-found で失敗しない
- **スクリプト**: tc-ta.js TC-926
