...
- **authRequired**: true (admin)
- **背景**: GP 予選のカップ割り当ては4カップのシャッフル済みデッキを5回つなげて作る。デッキ境界で前ラウンドと同じカップから始まる場合はローテーションし、通常設定では隣接ラウンドが同じカップにならない。将来カップリストが1種類などに変わった場合はこの制約を満たせないため、サーバー側は警告を出して防御的に検知する。
- **手順**:
...
### TC-1417: ホーム画面の sponsored recommendation 表示
- **URL**: `/`
- **背景**: issue #1417。ホーム画面に外部 Amazon recommendation を追加する場合、利用者が sponsored/PR 表示で判別でき、別タブ遷移時の link relationship も安全である必要がある。
- **手順**:
  1. ホーム画面 `/` を開く
  2. `recommended-heading` で示されるおすすめセクションが表示されることを確認する
  3. PR disclosure label が表示されることを確認する
  4. 商品名（例: Lenovo 14e Chromebook Gen 3）が表示されることを確認する
  5. Amazon 外部リンクの `href` と `target="_blank"` を確認する
  6. 外部リンクに `rel="sponsored noopener noreferrer"` が指定されていることを確認する
  7. issue #1431: この terminal セクション guard が空スライスを成功扱いしないことを静的テストで確認する
  8. issue #1433/#1434: terminal セクション guard の失敗メッセージが対象 marker を示し、静的テストが本文の日本語フレーズ変更に依存しないことを確認する
- **期待結果**: おすすめセクションが日英の翻訳キーを通じて商品名・説明・PR label を表示し、Amazon 外部リンクは sponsored disclosure と安全な `rel` 属性を維持する。terminal セクション guard は、次の `TC-` 見出しがない末尾セクションでも非空アサーションを維持し、失敗時に対象 marker を示す。TC-1417 の静的 guard は `issue #1431` と script metadata などの安定したキーで文書整合性を検証する。
- **スクリプト**: e2e/tc-all.js TC-1417 / `__tests__/static/tc-1417-home-recommendation.test.ts`
