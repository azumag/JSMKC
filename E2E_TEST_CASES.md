...
- **手順**:
  ...
  3. トーナメントタブから BM ページへ移動する
  4. URL が `/bm` に変わり、marker が残っていることを確認する
- **期待結果**: タブクリックは full document reload ではなく SPA navigation として完了する
- **スクリプト**: tc-all.js TC-939 / `__tests__/static/tc-939-tournament-tabs-link.test.ts`
...
- **背景**: issue #2040。TA phases API は preview/production D1 の `request-hung` 実績を避けるため、phase detail reads を `Promise.all` に戻さず sequential に保っている。コメントが「コメントを保持せよ」というメタ説明になると運用根拠が読み取りづらくなり、`has repeatedly produced` の観測事実が薄れる。
- **手順**:
  1. `src/app/api/tournaments/[id]/ta/phases/route.ts` の sequential D1 読み込みコメントを確認する
  2. コメントが `has repeatedly produced request-hung failures` と `latency trade-off` を直接説明していることを確認する
  3. コメント内に `Keep this ... behavior note` のようなメタコメントが残っていないことを確認する
  4. sequential read block が `retryDbRead` を使い、`Promise.all` を含まないことを確認する
- **期待結果**: sequential 読み込みの歴史的根拠と latency trade-off が直接説明され、メタコメントなしで回帰検知される
- **スクリプト**: `__tests__/docs/e2e-cases-drift.test.ts` / `__tests__/static/tc-2040-ta-phases-comment-history.test.ts`
