...
### TC-822: MR二重報告 — 管理者確定後のスコア変更不可 ❌ NOT IMPLEMENTED
- **URL**: /api/tournaments/[temp-id]/mr/matches/[matchId]
- **authRequired**: true (admin)
- **背景**: `scoresConfirmed` フィールドが MRMatch スキーマに存在しないため、この機能は未実装
- **現状**: MRMatch テーブルに `scoresConfirmed` カラムがない。dual report 時の管理者確定機能が必要であれば新規issueとして管理
- **対応**: 必要に応じて feature request issue を作成
