...
### TC-3011: TAバトルロイヤル専用モード — 4人・10ライフ・個別タイムハンデ
- **手順**:
  1. 4選手を作成し、TAハンデをそれぞれ0秒 / -1秒 / -3秒 / -5秒に設定する。
  2. 「TAバトルロイヤル専用モード」を有効にしたトーナメントを作成し、4選手をTA予選へ登録する。
  3. Phase 1/2を経由せずPhase 3を開始でき、全員が10ライフで登録されることを確認する。
  4. 生タイムでは遅いハンデ選手が補正後タイムで上位になる結果を入力し、補正後の順位で下位半分だけがライフを失うことを確認する。
  5. 人数が通常TAのライフ回復閾値（4人・2人）に達してもライフが回復しないことを確認する。
  6. 最後の1人まで進行し、優勝者が確定することを確認する。
  7. クリーンアップ: トーナメントとテスト選手を削除する。
- **期待結果**: 少人数TAをPhase 3へ直接進行でき、10ライフ・回復なし・0/-1/-3/-5秒の個別ハンデが生タイムを保持したまま順位とライフ減算へ反映される。通常TAのPhase 1/2/3進行と3ライフ制は変わらない。
- **スクリプト**: 未スクリプト化。ユニット/API/静的テストでルール・保存・UI配線を検証し、プレビュー反映後にPlaywright永続プロファイルで実地確認する。
