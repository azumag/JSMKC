...
- **期待結果**: おすすめセクションが日英の翻訳キーを通じて商品名・説明・PR label を表示し、Amazon 外部リンクは sponsored disclosure と安全な `rel` 属性を維持する。terminal セクション guard は、次の `TC-` 見出しがない末尾セクションでも非空アサーションを維持する。
