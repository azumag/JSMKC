...
- **手順**:
  1. `taFinals` と `taElimination` に `suddenDeathTiebreak` / `suddenDeathRoundDesc` / `suddenDeathCourse` / `submitSuddenDeath` があることを確認する
  2. Phase3 ページが `tTaFinals(...)` でサドンデス UI 文字列を解決していることを確認する
  3. Phase1/2 コンポーネントが `tElim(...)` でサドンデス UI 文字列と invalid time エラーを解決していることを確認する
